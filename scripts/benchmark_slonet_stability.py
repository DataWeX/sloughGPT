...
def load_slonet(checkpoint_path: str):
    ...
    def generate(prompt: str, max_tokens: int = 30, temperature: float = 0.7) -> str:
        ...
        input_ids = np.array(
            [[i % net.vocab_size for i in range(min(5, net.vocab_size))]], dtype=np.int64
        )
...
def run_benchmark(checkpoint_path: str, runs: int = 20, verbose: bool = False) -> StabilityResult:
    ...
    for i in range(runs):
        ...
        if verbose:
            if status == "ok":
                print(
                    f"  [{i + 1:2d}/{runs}] OK {latency * 1000:7.1f}ms  len={len(text)}  {text[:60]!r}"
                )
            else:
...
def print_report(result: StabilityResult, verbose: bool = False):
    ...
    print(
        f"  Crashes:            {len([r for r in result.records if r.status != 'ok'])}/{result.runs}  "
    ...
    print(
        f"  Empty responses:    {len(empties)}/{result.runs}  "
    ...
    if len(ok) >= 4:
        ...
        print("  -- Trend --")
        print(
            f"  First {half} avg: {f_avg * 1000:.0f}ms   Last {half} avg: {l_avg * 1000:.0f}ms   d {trend:+.0f}%"
        )
        if trend > 20:
            print("  WARN: Latency increasing - possible leak")
        elif trend < -10:
            print("  NOTE: Latency improving (warmup)")
