...
# MAN_API_PORT pins the API port (required for Next.js proxy to match).
set -euo pipefail
...
echo "Starting API: MAN_API_PORT=$MAN_API_PORT python3 apps/api/server/main.py"
...
for _ in $(seq 1 30); do
  ...
  if ! kill -0 "$API_PID" 2>/dev/null; then
    echo "❌ API process exited before /health responded. Fix errors above or free port $MAN_API_PORT."
    wait "$API_PID" || true
    exit 1
  fi
  sleep 1
