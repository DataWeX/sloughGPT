...
from domain.multimodal.engine import MultimodalEngine
...
print(
    f"  Loaded in {time.time() - t0:.1f}s — trained={engine._trained}, vocab={engine.text.vocab_size}"
)
print()
...
print("  Target:     'red circle on white background'")
print()
...
print("  Target:     'blue square on gray background'")
print()
...
print("=" * 50)
result_both = engine.generate(
    image_np=img.reshape(1, 224, 224, 3), audio_np=waveform, max_len=35, temperature=0.0
)
print(f"  Generated: '{result_both.text}'")
print("  Target:     'red circle on white background and middle A pitch'")
print()
