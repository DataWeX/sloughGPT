...
from domain.infrastructure._internal.numpy_ops import gelu, layer_norm, softmax
...
t0 = time.perf_counter()
for _ in range(N):
    qkv = h @ W_qkv + (b_qkv if b_qkv is not None else 0)
...
q2 = q.reshape(1, n_h, hd).transpose(1, 0, 2)
k2 = k.reshape(1, n_h, hd).transpose(1, 0, 2)
...
t0 = time.perf_counter()
for _ in range(N):
    out = (a @ v2).transpose(1, 0, 2).reshape(1, arch.n_embed)
...
W_up = w("layers.{i}.ffn.up.weight", 0)
b_up = wn("layers.{i}.ffn.up.bias", 0)
t0 = time.perf_counter()
for _ in range(N):
    h2 = h @ W_up + (b_up if b_up is not None else 0)
