...
def run():
    ...
    ops = [
        ...
        (
            "cat (2×1K)",
            lambda: ml.cat([ml.tensor(np.random.randn(1000).astype(np.float32)) for _ in range(2)]),
        ),
        (
            "stack (10×1K)",
            lambda: ml.stack(
                [ml.tensor(np.random.randn(1000).astype(np.float32)) for _ in range(10)]
            ),
        ),
        (
            "where (1K)",
        ...
        (
            "topk (1K, k=10)",
            lambda: ml.topk(ml.tensor(np.random.randn(1000).astype(np.float32)), 10),
        ),
        ("sort (1K)", lambda: ml.sort(ml.tensor(np.random.randn(1000).astype(np.float32)))),
        (
            "clamp (1K)",
            lambda: ml.clamp(ml.tensor(np.random.randn(1000).astype(np.float32)), min=-1, max=1),
        ),
        (
            "softmax (1K)",
            lambda: ml.softmax(ml.tensor(np.random.randn(1000).astype(np.float32)), dim=-1),
        ),
        (
            "matmul (64×128 × 128×64)",
            lambda: ml.matmul(
    ...
    portability_tests = [
        ...
        (
            "isnan(arr)",
            lambda: ml.isnan(ml.tensor(np.array([1.0, float("nan")], dtype=np.float32))),
        ),
        (
            "isinf(arr)",
            lambda: ml.isinf(ml.tensor(np.array([1.0, float("inf")], dtype=np.float32))),
        ),
        (
            "cat([a,b])",
            lambda: ml.cat(
                [
                    ml.tensor(np.array([1.0], dtype=np.float32)),
                    ml.tensor(np.array([2.0], dtype=np.float32)),
                ]
            ),
        ),
        (
            "where(cond,x,y)",
            lambda: ml.where(
                ml.tensor(np.array([True], dtype=ml.bool)),
                ml.tensor(np.array([1.0], dtype=np.float32)),
                ml.tensor(np.array([0.0], dtype=np.float32)),
            ),
        ),
        (
            "topk(arr, 2)",
            lambda: ml.topk(ml.tensor(np.array([3.0, 1.0, 2.0], dtype=np.float32)), 2),
        ),
        ("sort(arr)", lambda: ml.sort(ml.tensor(np.array([3.0, 1.0, 2.0], dtype=np.float32)))),
        (
            "clamp(arr, 0, 1)",
            lambda: ml.clamp(ml.tensor(np.array([0.5, 1.5, -0.5], dtype=np.float32)), min=0, max=1),
        ),
        (
            "softmax(arr)",
            lambda: ml.softmax(ml.tensor(np.array([1.0, 2.0, 3.0], dtype=np.float32)), dim=-1),
        ),
        (
            "matmul(a,b)",
            lambda: ml.matmul(
                ml.tensor(np.array([[1.0], [2.0]], dtype=np.float32)),
                ml.tensor(np.array([[3.0, 4.0]], dtype=np.float32)),
            ),
        ),
    ...
    print(
        f"  Portability:    {passed}/{len(portability_tests)} ({100 * passed / len(portability_tests):.0f}%)"
    )
    print(f"  Module size:    {module_size / 1024:.1f} KB")
    print("  No torch deps:  All ops work without torch")
    print(f"{'=' * 78}")
