/** React hooks for the cache system.
...
export function useQuery<T>(
  ...
  const getSnapshot = useCallback((): QueryState<T> => {
    ...
    if (
      s.data === (entry.data as T) &&
      s.error === entry.error &&
      s.status === entry.status &&
      s.isFetching === entry.isFetching &&
      s.updatedAt === entry.updatedAt
    ) {
      return s
    }
    snapRef.current = {
...
export function useMutation<T, V = void>(
  ...
  const mutateAsync = useCallback(async (vars: V): Promise<T> => {
  ...
  const mutate = useCallback(
    (vars: V) => {
      mutateAsync(vars).catch((e) => logger.error('[mutation] failed', { exception: String(e) }))
    },
    [mutateAsync],
  )
...
export function useIsFetching(): number {
  const subscribe = useCallback((cb: () => void) => useQueryStore.subscribe(cb), [])
  const getSnapshot = useCallback(() => useQueryStore.getState().fetchingKeys.size, [])
