...
// Mode colors use theme tokens only (shared with EmptyState cards — same
// mode, same token). Only one tab is active at a time, so token reuse
// across modes is intentional and keeps a single accent language.
const MODES: { value: ChatMode; label: string; icon: ReactNode; color: string }[] = [
  ...
  },
  {
    value: 'talk',
    label: 'Talk',
    icon: <IconMic className="h-3.5 w-3.5" />,
    color: 'bg-primary/10 text-primary border-primary/20',
  },
]
...
function SubOptions({
  ...
  onDecideStructureChange,
  onDifficultyChange,
  onLangPairChange,
  onBrainstormTopicChange,
...
>) {
    ...
    return (
      <div className="flex items-center gap-3 px-3 py-1.5 border-b border-border/15 bg-muted/30">
        <div className="flex items-center gap-1">
          ...
          {TONES.map((t) => (
            <SubOptionPill key={t} active={tone === t} label={t} onClick={() => onToneChange(t)} />
          ))}
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          {TYPES.map((t) => (
            <SubOptionPill key={t} active={type === t} label={t} onClick={() => onTypeChange(t)} />
          ))}
        </div>
      </div>
    )
      ...
      <div className="flex items-center gap-3 px-3 py-1.5 border-b border-border/15 bg-muted/30">
        <div className="flex items-center gap-1">
          ...
          {REWRITE_OPTIONS.map((o) => (
            <SubOptionPill
              key={o}
              active={rewriteStyle === o}
              label={o}
              onClick={() => onRewriteStyleChange(o)}
            />
          ))}
        </div>
      </div>
  ...
  > = {
    decide: [
      {
        label: 'Output',
        options: DECIDE_STRUCTURES,
        value: decideStructure,
        onChange: onDecideStructureChange,
      },
    ],
    explain: [
      { label: 'Level', options: DIFFICULTIES, value: difficulty, onChange: onDifficultyChange },
    ],
    translate: [{ label: 'To', options: LANG_PAIRS, value: langPair, onChange: onLangPairChange }],
    brainstorm: [
      {
        label: 'Topic',
        options: BRAINSTORM_TOPICS,
        value: brainstormTopic,
        onChange: onBrainstormTopicChange,
      },
    ],
    wellness: [
      {
        ...
        value: wellnessType,
        onChange: onWellnessTypeChange,
      },
    ],
    create: [
      { label: 'Style', options: CREATE_STYLES, value: createStyle, onChange: onCreateStyleChange },
    ],
  }
  ...
  return (
    <>
      ...
      <SubOptions
        ...
        onDecideStructureChange={onDecideStructureChange}
        onDifficultyChange={onDifficultyChange}
        onLangPairChange={onLangPairChange}
        onBrainstormTopicChange={onBrainstormTopicChange}
        onWellnessTypeChange={onWellnessTypeChange}
        onCreateStyleChange={onCreateStyleChange}
      />
    </>
  )
})
