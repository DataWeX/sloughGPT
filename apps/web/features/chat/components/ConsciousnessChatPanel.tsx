...
/**
 * ConsciousnessChatPanel — side panel that displays consciousness state.
 *
 * This is a thin consumer of ConsciousnessProvider. All state comes from
 * useConsciousness() — no local state management for consciousness data.
 */
...
export const ConsciousnessChatPanel = memo(function ConsciousnessChatPanel({
  open,
  onClose,
}: ConsciousnessChatPanelProps) {
  const { t } = useLocale()
  const [expanded, setExpanded] = useState(false)
  const [rating, setRating] = useState(0)

  // All consciousness state from the provider
  const {
    event,
    level,
    qualia,
    ...
    >
      <div className="flex items-center justify-between px-3 py-2 border-b border-border/50 shrink-0">
        ...
        <Button
          ...
          className="h-6 w-6"
          onClick={onClose}
          aria-label="Close consciousness panel"
        >
        ...
        <section aria-label={t('consciousness_chat.current_state')}>
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
              {t('consciousness_chat.current_state')}
            </span>
          </div>
          ...
          {event ? (
            <div className="space-y-2">
              <div className="space-y-1.5">
                {Object.entries(qualia).map(([name, value]) => (
                ...
                <>
                  <div className="border-t border-border/30 pt-2 mt-2">
                    <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider block mb-1.5">
                      {t('consciousness_chat.beliefs')}
                    </span>
                    <div className="space-y-1">
                      {Object.entries(beliefs).map(([name, value]) => (
                  ...
                  <div className="border-t border-border/30 pt-2 mt-2">
                    <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider block mb-1">
                      {t('consciousness_chat.growth_delta')}
                    </span>
                    <div className="flex items-center gap-2">
                      <span
                        className={cn(
                          'text-sm font-mono font-medium',
                          growthDelta >= 0 ? 'text-success' : 'text-destructive',
                        )}
                      >
                        {growthDelta >= 0 ? '+' : ''}
          ...
          <>
            <section aria-label={t('consciousness_chat.episode_info')}>
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                  {t('consciousness_chat.episode_info')}
                </span>
              </div>
              {episodes.length > 0 ? (
                <div className="space-y-2">
                  {episodes.map((ep, i) => (
                    <div
                      key={i}
                      className="p-2 rounded bg-muted/30 border border-border/40 space-y-1"
                    >
                      {ep.narrative && (
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={cn(
                            'text-[9px] font-mono',
                            ep.growth_delta >= 0 ? 'text-success' : 'text-destructive',
                          )}
                        >
                          {ep.growth_delta >= 0 ? '+' : ''}
            ...
            <section aria-label={t('consciousness_chat.quick_actions')}>
              <div className="flex items-center gap-1.5 mb-2">
                <IconSparkle className="h-3 w-3 text-muted-foreground" />
                <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                  {t('consciousness_chat.quick_actions')}
                </span>
              </div>
              <div className="space-y-2">
                <Button
                ...
                >
                  ...
                  {reflecting
                    ? t('consciousness_chat.reflecting')
                    : t('consciousness_chat.reflect')}
                </Button>

                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-muted-foreground shrink-0">
                    {t('consciousness_chat.rate')}
                  </span>
