...
describe('TokenTreePage', () => {
  ...
  it('compares tokens', async () => {
    ...
    render(<TokenTreePage />)
    const compareTabs = screen.getAllByText('Compare')
    fireEvent.click(compareTabs[0])
    await waitFor(() => {
      expect(screen.getByText('Compare Tokens')).toBeInTheDocument()
    })
    const textboxes = screen.getAllByRole('textbox')
    fireEvent.change(textboxes[textboxes.length - 2], { target: { value: 'tree' } })
