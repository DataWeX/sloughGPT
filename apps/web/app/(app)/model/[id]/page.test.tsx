...
vi.mock('@sloughgpt/strui', () => {
  ...
  return {
    ...
    Badge: ({ label, variant, size, className, children }: any) => (
    ...
    KeyValueList: ({ items }: any) => (
      <div>
        {items?.map((item: any, i: number) => (
          <div key={i} data-testid={`kv-${item.label}`}>
    ...
    SettingsRow: ({ title, control, children }: any) => (
      <div>
        <span>{title}</span>
        {control || children}
      </div>
...
vi.mock('@/components/model/QuantizeCard', () => ({
  QuantizeCard: ({ isLoaded, modelId }: any) => (
    <div data-testid="quantize-card" data-loaded={isLoaded}>
      Quantize
    </div>
  ),
}))
...
vi.mock('@/components/PageContainer', () => ({
  PageContainer: ({ title, children, loading, loadingContent, loadingCards }: any) => (
    <div className="sl-page mx-auto max-w-4xl">
      <h1>{loading ? '...' : title}</h1>
      {loading
        ? (loadingContent ?? (loadingCards ? <div data-testid="skeleton" /> : null))
        : children}
    </div>
  ),
}))
...
describe('ModelDetailPage', () => {
  ...
  it('shows unloaded status for non-ready model', async () => {
    mocks.apiGet.mockImplementation((url: string) => {
      if (url.includes('/registry/models/'))
        return Promise.resolve({ ...SAMPLE_MODEL, status: 'unloaded' })
      return Promise.resolve({ logs: [] })
    })
    render(<Page />)
    await waitFor(() => {
  ...
  it('navigates to training', async () => {
    render(<Page />)
    await waitFor(() => {
      expect(screen.getByText('Quick Actions')).toBeTruthy()
    })
    fireEvent.click(screen.getByText('Training'))
    expect(stableRouter.push).toHaveBeenCalledWith('/training')
  })
