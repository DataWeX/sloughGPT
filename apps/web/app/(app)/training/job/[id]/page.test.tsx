...
vi.mock('next/navigation', () => ({ useParams: () => ({ id: 'job-1' }), useRouter: () => stableRouter, useSearchParams: () => new URLSearchParams(), usePathname: () => '/training/job/job-1' }))
...
describe('TrainingJobDetailPage', () => {
  ...
  it('shows error message when loading fails', async () => {
    ...
    render(<Page />)
    await waitFor(() => { expect(screen.getByText(/Failed to load training job/)).toBeTruthy() })
  })
