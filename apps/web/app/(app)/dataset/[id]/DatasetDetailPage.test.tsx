...
const { mockGet, mockUpdate, mockDelete, mockExport, mockPush, mockAddToast } = vi.hoisted(() => ({
  mockGet: vi.fn(), mockUpdate: vi.fn(), mockDelete: vi.fn(),
  mockExport: vi.fn(), mockPush: vi.fn(), mockAddToast: vi.fn(),
}))
const stableRouter = { push: mockPush }

vi.mock('next/navigation', () => ({ useRouter: () => stableRouter, useParams: () => ({ id: 'shakespeare' }) }))
vi.mock('@/lib/dataset-controller', () => ({ datasetController: { get: mockGet, update: mockUpdate, delete: mockDelete, export: mockExport } }))
