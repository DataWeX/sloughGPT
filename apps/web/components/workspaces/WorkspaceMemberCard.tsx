...
interface WorkspaceMemberCardProps {
  ...
  onAddMemberRoleChange: (role: string) => void
  onAddMember: () => void
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        <div className="flex gap-1.5">
          ...
          <select
            value={addMemberRole}
            onChange={(e) => onAddMemberRoleChange(e.target.value)}
          ...
          <Button size="sm" className="h-6 text-[10px]" onClick={onAddMember} disabled={!addMemberId.trim()}>
            Add
          </Button>
        </div>
