...
describe('DatasetSelector', () => {
  ...
  it('hides adapter/system entries from training selection', () => {
    const withData = {
      ...datasets,
      datasets: [
        ds({ id: 'tiny', name: 'tiny' }),
        ds({ id: 'user_adapters', name: 'user adapters', kind: 'adapter' }),
        ds({ id: 'rag_store', name: 'rag store', kind: 'system' }),
      ],
    }
    const { container } = render(
      <DatasetSelector datasets={withData} value="" onChange={vi.fn()} />,
    )
    const options = [...container.querySelectorAll('option')].map((o) => o.getAttribute('value'))
    expect(options).toEqual(['tiny'])
  })

  it('shows all kinds when showAllKinds is set', () => {
    const withData = {
      ...datasets,
      datasets: [
        ds({ id: 'tiny', name: 'tiny' }),
    ...
    const { container } = render(
      <DatasetSelector datasets={withData} value="" onChange={vi.fn()} showAllKinds />,
    )
    const options = [...container.querySelectorAll('option')].map((o) => o.getAttribute('value'))
    expect(options).toEqual(['tiny', 'user_adapters'])
  })
})
