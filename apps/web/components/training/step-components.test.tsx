...
vi.mock('@sloughgpt/strui', () => {
  ...
  return {
    ...
    ToggleGroupItem: ({ value, children, ...props }: any) => (
      <button data-value={value} onClick={() => props.onClick?.()}>
        {children}
      </button>
    ),
    ...
    EmptyCard: ({ title, description, icon }: any) => (
...
describe('DataStep', () => {
  ...
  it('renders card title', () => {
    render(
      <DataStep form={makeForm()} datasets={makeDatasets()} onNext={vi.fn()} onBack={vi.fn()} />,
    )
  ...
  it('hides helper text when data is ready', () => {
    render(
      <DataStep
        form={makeForm()}
        datasets={makeDatasets({ selectedDataset: 'ds-1' })}
        onNext={vi.fn()}
        onBack={vi.fn()}
      />,
    )
  ...
  it('enables Next when text input is non-empty', () => {
    render(
      <DataStep
        form={makeForm({ inputMode: 'text', textInput: 'hello' })}
  ...
  it('shows preview when datasetPreview is provided', () => {
    ...
    render(
      <DataStep
        form={makeForm()}
        datasets={makeDatasets({ datasetPreview: preview })}
        onNext={vi.fn()}
        onBack={vi.fn()}
      />,
    )
...
describe('TrainStep', () => {
  ...
  it('renders card title', () => {
    render(
      <TrainStep form={makeForm()} datasets={makeDatasets()} onNext={vi.fn()} onBack={vi.fn()} />,
    )
  ...
  it('shows config summary', () => {
    render(
      <TrainStep
        form={makeForm({ trainingEpochs: 5, trainingBatchSize: 16 })}
        datasets={makeDatasets({ selectedDataset: 'ds-1' })}
        onNext={vi.fn()}
        onBack={vi.fn()}
      />,
  ...
  it('calls startTraining when Start is clicked', async () => {
    ...
    render(
      <TrainStep
        form={makeForm({ canStart: true, startTraining })}
        datasets={makeDatasets()}
        onNext={vi.fn()}
        onBack={vi.fn()}
      />,
  ...
  it('calls onBack when Back is clicked', () => {
    const onBack = vi.fn()
    render(
      <TrainStep form={makeForm()} datasets={makeDatasets()} onNext={vi.fn()} onBack={onBack} />,
    )
  ...
  it('shows "Train on pasted text" for distill+text', () => {
    render(
      <TrainStep
        form={makeForm({ method: 'distill', inputMode: 'text', textInput: 'some text' })}
        datasets={makeDatasets()}
        onNext={vi.fn()}
        onBack={vi.fn()}
      />,
...
describe('ResultsStep', () => {
  ...
  it('shows checkpoints when available', () => {
    ...
    expect(screen.getByText('1 checkpoint(s) saved')).toBeTruthy()
  })
