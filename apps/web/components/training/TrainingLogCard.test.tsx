...
import { TrainingLogCard, summarizeLogLines } from './TrainingLogCard'
...
describe('TrainingLogCard', () => {
  const mockGetTrainingLog = mockTrainingLog
  ...
  it('shows loading skeleton while fetching', async () => {
    ...
    mockGetTrainingLog.mockImplementation(
      () =>
        new Promise((r) => {
          resolveFn = r
        }),
  ...
  it('shows concise metric cells parsed from logs', async () => {
    mockGetTrainingLog.mockResolvedValue([
      'epoch 2 step 120 train_loss 1.2044 lr 0.001',
      'step 121 train_loss 1.1987',
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('1.1987')).toBeTruthy()
    })
...
describe('summarizeLogLines', () => {
  it('takes newest value per metric and counts errors', () => {
    const s = summarizeLogLines(['step 1 loss 2.5', 'step 2 loss 2.1 ERROR boom', 'warn: x'])
    expect(s).toMatchObject({ step: '2', loss: '2.1', errors: 1, warnings: 1 })
  })

  it('returns nulls on empty input', () => {
    expect(summarizeLogLines([])).toMatchObject({ step: null, loss: null, errors: 0, warnings: 0 })
  })
})
