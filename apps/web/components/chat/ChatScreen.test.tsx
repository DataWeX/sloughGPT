...
vi.mock('./MessageBubble', () => ({ MessageBubble: ({ content, role }: any) => <div data-testid="msg-bubble" data-role={role}>{content}</div> }))
vi.mock('./EmptyState', () => ({ EmptyState: ({ hasModel, onSuggestionClick }: any) => <div data-testid="empty-state" data-has-model={hasModel} /> }))
vi.mock('./SystemBanner', () => ({ SystemBanner: ({ type, title }: any) => <div data-testid="sys-banner" data-type={type}>{title}</div> }))
