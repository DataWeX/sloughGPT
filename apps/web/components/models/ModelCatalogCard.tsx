...
import { useLoadModel } from '@/lib/cache/api-hooks'
...
export default memo(function ModelCatalogCard({
  models,
  modelsLoading,
  activeRuntimeId,
  onModelLoaded,
        ...
        {modelsLoading ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="animate-pulse rounded-lg bg-muted/50 border border-border/50 p-4 h-20"
              />
            ))}
          </div>
        ) : models.length === 0 ? (
          ...
          <>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 max-h-[60vh] overflow-y-auto overscroll-contain">
              {sorted.map((model) => {
                ...
                const isLoaded = activeRuntimeId
                  ? catalogIdMatchesRuntime(model.id, activeRuntimeId)
                  : false
                ...
                return (
                  <div key={model.id} className="group">
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') router.push(`/model/${encodeURIComponent(model.id)}`)
                      }}
                      tabIndex={0}
                      role="button"
                    >
                      <div className="min-w-0 flex-1">
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              isFav
                                ...
                                : 'opacity-0 group-hover:opacity-60 hover:!opacity-100 text-muted-foreground/40',
                            )}
                        ...
                        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                          ...
                          {!isLoaded && model.cached && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-success/10 text-success font-medium">
                              Cached
                            </span>
                          )}
                          {model.archive_format === 'lz4' && (
                            <span
                              className="text-[10px] px-1.5 py-0.5 rounded bg-info/10 text-info font-medium"
                              title="Safetensors archived as LZ4"
                            >
                              LZ4
                            </span>
                          )}
                          {loadTimes[model.id] && (
                            <span className="text-[10px] text-muted-foreground/60">
                              {(loadTimes[model.id] / 1000).toFixed(1)}s
                            </span>
                          )}
                          {model.tags &&
                    ...
                    {isLoading &&
                      conversionStatus &&
                      conversionStatus.stage !== 'idle' &&
                      conversionStatus.stage !== 'ready' && (
                        <div key={`${model.id}-progress`} className="col-span-full mt-1">
                          ...
                          <span className="text-[9px] text-muted-foreground ml-auto">
                            {conversionStatus.elapsed_s.toFixed(0)}s
                          </span>
                        </div>
                      )}
                  </div>
