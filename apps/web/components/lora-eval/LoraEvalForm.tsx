      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input value={adapterPath} onChange={e => onAdapterPathChange(e.target.value)} className="h-8 text-xs font-mono mt-1" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">Soul</label>
            <Input value={soul} onChange={e => onSoulChange(e.target.value)} className="h-8 text-xs mt-1" />
          </div>
        </div>
