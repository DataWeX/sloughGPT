...
def register(cli):
    ...
    @click.option(
        "--corpus",
        "-c",
        default="datasets/tinyshakespeare/input.txt",
        help="Corpus file or dataset name",
    )
    ...
    @click.option(
        ...
        default=64,
        type=int,
        help="Embedding dimension (0 disables embeddings)",
    )
    @click.option("--min-freq", default=2, type=int, help="Minimum pair frequency to merge")
    @click.option(
        "--output", "-o", default="models/slonet-native/token_tree", help="Save base path"
    )
    @click.pass_context
    ...
    @click.option(
        "--tree", "-t", default="models/slonet-native/token_tree", help="Saved tree base path"
    )
    ...
    @click.option(
        "--limit", "-n", default=50, type=int, help="Maximum entries to print (0 = no limit)"
    )
    @click.pass_context
    ...
    @token_tree.command(
        "path", help="Trace the greedy trie walk over text (reads stdin when no --text)"
    )
    @click.option(
        "--tree", "-t", default="models/slonet-native/token_tree", help="Saved tree base path"
    )
    ...
    @click.option(
        "--top-k", "-k", default=10, type=int, help="Shared/exclusive token examples per side"
    )
    @click.pass_context
    ...
    @click.option(
        "--query", "-q", default="", help="Filter rules whose parts contain this substring"
    )
    @click.pass_context
