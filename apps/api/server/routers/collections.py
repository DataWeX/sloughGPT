...
class PipelineConfigRequest(BaseModel):
    ...
    source_type: str = Field(
        ..., description="Source type: file, url, rss, api, sse, watch, generator"
    )
    source_config: dict = Field(default_factory=dict)
    store_type: str = Field(
        default="memory", description="Store type: file, memory, callback, chained, stats"
    )
    store_config: dict = Field(default_factory=dict)
    filter_chain: list[dict] = Field(
        default_factory=list, description="Ordered list of filter configs"
    )
    ...
    async def create_pipeline(
        ...
        try:
            ...
            safe_audit_log(
                "collection.create",
                resource=req.name,
                detail=f"source={req.source_type} store={req.store_type}",
            )
            return success_response(
                data={
                    "id": req.name,
    ...
    async def run_pipeline(
        ...
        try:
            ...
            safe_audit_log(
                "collection.run",
                resource=name,
                detail=f"collected={count} elapsed={_elapsed_ms:.0f}ms",
            )
            return success_response(
                data={
                    "pipeline": name,
