...
class TokenizerRouter:
    ...
    async def get_tokenizer_stats(self):
    ...
    async def detokenize_ids(self, req: DetokenizeRequest):
        self._require_trained()
        mgr = get_tokenizer_manager()
        text = mgr.detokenize(req.ids)
        return success_response(data={"text": text})

    async def get_vocab(self, limit: int = 50, offset: int = 0):
        self._require_trained()
    ...
    async def get_merges(self, limit: int = 30):
        self._require_trained()
        tok = get_tokenizer_manager().get_tokenizer()
    ...
    async def train_tokenizer(self, req: TrainTokenizerRequest2):
        """
