...
## Global Banner System (frontend engineering rule)

One banner surface for the entire app — `useBannerStore` (`apps/web/lib/banner-store.ts`)
rendered once as `<GlobalBanner />` in `AppLayout`. It connects all pages: a banner
raised anywhere survives navigation until dismissed or acted on.

- Toasts = transient confirmations (auto-expire, 6s). Banners = journey blockers
  (training failed, empty dataset, backend down) with optional action buttons.
- Never build a second banner surface per page — route through the store.
  Use `key` to dedupe (re-showing the same key replaces, never stacks).
- Tones map to theme tokens via `StatusDot` (info→primary, success, warning,
  destructive). No new colors.
