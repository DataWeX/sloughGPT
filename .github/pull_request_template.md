...
- [ ] If **repo root `package.json`** changed (e.g. **`dev:stack`** / **`dev-stack.sh`**): `npm run test:repo-root` (after `npm install` at repo root), `make test-repo-root`, or `python3 -m pytest tests/test_repo_root_package_json.py -q`
