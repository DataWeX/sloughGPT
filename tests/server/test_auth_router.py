...
class TestRegister:
    def test_registers_user(self, router, client):
        ...
        router._get_auth_deps = MagicMock(return_value=(["key"], 24, _make_jwt_mock(), MagicMock()))
        resp = client.post(
            "/auth/register",
            json={
                "username": "alice",
...
class TestLogin:
    def test_login_success(self, router, client):
        router._users.find_one = MagicMock(
            return_value={
                "_id": "uid1",
                "username": "alice",
                "email": "a@b.com",
...
class TestToken:
    def test_creates_token_for_valid_key(self, router, client):
        jwt = _make_jwt_mock()
        router._get_auth_deps = MagicMock(return_value=(["valid-key"], 24, jwt, MagicMock()))
    ...
    def test_expires_in_is_hours_seconds(self, router, client):
        jwt = _make_jwt_mock()
        router._get_auth_deps = MagicMock(return_value=(["valid-key"], 2, jwt, MagicMock()))
        resp = client.post("/auth/token", json={"api_key": "valid-key"})
...
class TestGetMe:
    ...
    def test_me_with_valid_token(self, router, client):
        ...
        router._get_auth_deps = MagicMock(return_value=(["key"], 24, jwt, MagicMock()))
        router._users.find_one = MagicMock(
            return_value={"_id": "uid1", "username": "alice", "email": "a@b.com"}
        )
...
class TestRefreshToken:
    ...
    def test_refresh_returns_expiry(self, router, client):
        jwt = _make_jwt_mock()
        jwt.refresh_token.return_value = "new_jwt"
        router._get_auth_deps = MagicMock(return_value=(["key"], 3, jwt, MagicMock()))
