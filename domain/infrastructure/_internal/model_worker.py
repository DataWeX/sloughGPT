...
def _worker_loop(
    ...
    while True:
        ...
        if cmd == "stop":
            logger.info("Worker[%s]: stop requested", worker_id, extra={"tag": "INFRA"})
            break
        ...
        if cmd == "generate":
            ...
            try:
                ...
                logger.error(
                    "Worker[%s]: generate error: %s\n%s", worker_id, e, tb, extra={"tag": "INFRA"}
                )
                try:
                    resp_q.put_nowait(("error", session_id, f"{type(e).__name__}: {e}"))
                except Exception:
                    logger.error(
                        "Worker[%s]: generate error fallback also failed",
                        worker_id,
                        extra={"tag": "INFRA"},
                    )
        ...
        if cmd == "generate_stream":
            ...
            try:
                ...
                tb = traceback.format_exc()
                logger.error(
                    "Worker[%s]: stream error: %s\n%s", worker_id, e, tb, extra={"tag": "INFRA"}
                )
                try:
                    resp_q.put_nowait(("error", session_id, f"{type(e).__name__}: {e}"))
                except Exception:
                    logger.error(
                        "Worker[%s]: stream error fallback also failed",
                        worker_id,
                        extra={"tag": "INFRA"},
                    )
        ...
        if cmd == "load_adapter":
            ...
            try:
                ...
                if adapter_fn is None:
                    resp_q.put_nowait(
                        ("error", session_id, "Adapter loading not supported by this worker.")
                    )
                else:
                ...
                try:
                    ...
                    logger.error(
                        "Worker[%s]: load_adapter error fallback also failed",
                        worker_id,
                        extra={"tag": "INFRA"},
                    )
        ...
        if cmd == "unload_adapter":
            ...
            try:
                ...
                if unload_fn is None:
                    resp_q.put_nowait(
                        ("error", session_id, "Adapter unloading not supported by this worker.")
                    )
                else:
                    result = unload_fn()
                    resp_q.put_nowait(("result", session_id, result))
                ...
                tb = traceback.format_exc()
                logger.error(
                    "Worker[%s]: unload_adapter error: %s\n%s",
                    worker_id,
                    e,
                    tb,
                    extra={"tag": "INFRA"},
                ...
                try:
                    ...
                    logger.error(
                        "Worker[%s]: unload_adapter error fallback also failed",
                        worker_id,
                        extra={"tag": "INFRA"},
                    )
    ...
    if cleanup_fn:
        try:
            ...
            logger.warning(
                "Worker[%s]: cleanup_fn failed: %s", worker_id, e, extra={"tag": "INFRA"}
            )
    gc.collect()
    hb_q.put_nowait(("dead", os.getpid()))
    logger.info("Worker[%s]: stopped", worker_id, extra={"tag": "INFRA"})
...
def _slo_worker_main(
    ...
    logger.info("Worker[%s]: started (pid=%d, slo)", worker_id, os.getpid(), extra={"tag": "INFRA"})
    ...
    try:
        ...
        logger.info(
            "Worker[%s]: SloNet model loaded (%s)", worker_id, model_id, extra={"tag": "INFRA"}
        )
    except Exception as e:
        logger.error("Worker[%s]: SloNet load failed: %s", worker_id, e, extra={"tag": "INFRA"})
    ...
    def _load_adapter(adapter_path: str, merge: bool = False) -> dict:
        ...
        alpha = (
            float(adapter.get("_config/alpha", [16.0])[0]) if "_config/alpha" in adapter else 16.0
        )

        target_modules = []
        n_modules = (
            int(adapter.get("_config/target_modules", [0])[0])
            if "_config/target_modules" in adapter
            else 0
        )
        ...
        logger.info(
            "Worker loaded LoRA adapter %s (rank=%d, alpha=%.1f, %d params)",
            p,
            rank,
            alpha,
            n_params,
    ...
    def _unload_adapter() -> dict:
        ...
        logger.info(
            "Worker unloaded LoRA adapter, reverted to: %s", slnc_path, extra={"tag": "TRAIN"}
        )
        return {
            "status": "unloaded",
            "model_type": "slonet",
            "message": f"Reverted to base: {Path(slnc_path).name}",
        }

    _worker_loop(
        req_q, resp_q, hb_q, worker_id, _generate, _stream, _cleanup, _load_adapter, _unload_adapter
    )
...
def _hf_worker_main(
    ...
    if extra_sys_paths:
        for p in extra_sys_paths:
            if p not in sys.path:
                sys.path.insert(0, p)
    logger.info("Worker[%s]: started (pid=%d, hf)", worker_id, os.getpid(), extra={"tag": "INFRA"})
    ...
    try:
        ...
        logger.info(
            "Worker[%s]: HF model loaded (type=%s)",
            worker_id,
            type(model).__name__,
            extra={"tag": "INFRA"},
        )
        ...
        logger.error("Worker[%s]: HF model load failed: %s", worker_id, e, extra={"tag": "INFRA"})
...
class ModelWorkerProcess:
    ...
    def start(self) -> None:
        ...
        if self._process is not None and self._process.is_alive():
            logger.warning("Worker[%s]: already running", self.worker_id, extra={"tag": "INFRA"})
            return
    ...
    def stop(self, timeout: float = 10.0) -> None:
        ...
        try:
            ...
            logger.warning(
                "Worker[%s]: failed to send stop signal: %s",
                self.worker_id,
                e,
                extra={"tag": "INFRA"},
            )
        ...
        if self._process.is_alive():
            logger.warning(
                "Worker[%s]: killing unresponsive process", self.worker_id, extra={"tag": "INFRA"}
            )
            self._process.kill()
            self._process.join(timeout=5)
        self._health.alive = False
        self._cleanup_queues()
        logger.info("Worker[%s]: stopped", self.worker_id, extra={"tag": "INFRA"})
        ...
        **kwargs: Any,
        ...
        while time.time() < deadline:
            ...
            if msg == "result":
                ...
                logger.warning(
                    "Worker[%s]: unknown response type: %s",
                    self.worker_id,
                    msg,
                    extra={"tag": "INFRA"},
                )
    ...
    @staticmethod
    def _split_resp(msg: str, rest: list) -> tuple[str | None, Any]:
        ...
        **kwargs: Any,
        ...
        while time.time() < deadline:
            if not self.alive:
                self._health.crashed = True
                self._health.crash_count += 1
                raise RuntimeError(f"Worker[{self.worker_id}] crashed during streaming generation")
    ...
    def load_adapter(
        self, adapter_path: str, merge: bool = False, timeout: float = DEFAULT_GENERATE_TIMEOUT
    ) -> dict:
