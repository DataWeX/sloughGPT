...
class RequestCoalescer:
    ...
    async def start(self, key: str) -> _InFlightRequest | None:
        ...
        async with self._lock:
            ...
            if existing is not None:
                logger.debug(
                    "Coalescer hit: key=%s, age=%.1fs", key, time.monotonic() - existing.created_at
                )
                return existing
            ...
            logger.debug(
                "Coalescer registered: key=%s, total_in_flight=%d", key, len(self._in_flight)
            )
            return None
    ...
    async def _cleanup_stale(self) -> None:
        ...
        async with self._lock:
            now = time.monotonic()
            stale = [k for k, v in self._in_flight.items() if now - v.created_at > self._ttl]
            for k in stale:
            ...
            if len(self._in_flight) > self._max_entries:
                ...
                for k in sorted_keys[:evicted_count]:
                    ...
                    if not entry.event.is_set():
                        ...
                        entry.event.set()
                logger.warning(
                    "Coalescer evicted %d entries (max_entries=%d)",
                    evicted_count,
                    self._max_entries,
                )
