...
def _bytes_to_unicode() -> tuple[dict[int, str], dict[str, int]]:
    ...
    for b in range(2**8):
        if b not in bs:
            ...
            cs.append(2**8 + n)
            n += 1
    byte2unicode = dict(zip(bs, [chr(c) for c in cs], strict=False))
...
class MorphTokenizer:
    ...
    def from_pretrained(cls, model_id: str) -> MorphTokenizer:
        ...
        if tokenizer_path is None or not tokenizer_path.exists():
            raise FileNotFoundError(
                f"No tokenizer.json for {model_id} (searched {len(search_dirs)} dirs)"
            )
        ...
        if added_tokens:
            logger.info(
                "Loaded %d added tokens for %s: %s",
                len(added_tokens),
                model_id,
                list(added_tokens.keys())[:5],
                extra={"tag": "INFRA"},
            )

        logger.info(
            "Loaded tokenizer %s (vocab=%d, merges=%d, byte_level=%s, byte_fallback=%s, added=%d)",
            model_id,
            len(vocab),
            len(merges),
            byte_level,
        ...
        if chat_tpl:
            ...
            logger.info("Loaded chat template for %s", model_id, extra={"tag": "INFRA"})
    ...
    def morphological_diversity(self, words: list[str]) -> float:
        roots = {self.stem(w) for w in words}
