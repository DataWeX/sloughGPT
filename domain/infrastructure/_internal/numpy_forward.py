...
from .numpy_ops import gelu, layer_norm, rmsnorm, rope, silu, softmax
...
def forward(weights: dict, arch: ArchConfig, token_ids: list[int]) -> np.ndarray:
    ...
    def wn(canonical: str, layer: int = 0) -> np.ndarray | None:
    ...
    norm = norm_fn(arch)
    T = (
        (lambda w: w.T) if not arch.transpose_weights else (lambda w: w)
    ...
    if arch.norm == "rms_norm":
        ...
        x = norm(
            x, w("final_norm.weight"), b if b is not None else np.zeros(arch.n_embed, dtype=x.dtype)
        )
...
def forward_cached(
    ...
    if kv_cache is not None and kv_cache.seq_len > 0:
        ...
        mask = np.triu(
            np.full((seq_len, total_len), -1e10, dtype=np.float32), k=1 + kv_cache.seq_len
        )
    else:
        mask = np.triu(np.full((seq_len, seq_len), -1e10, dtype=np.float32), k=1)
