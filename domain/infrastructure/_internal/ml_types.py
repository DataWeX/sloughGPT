...
Usage:
    from .ml_types import dtype, device, tensor, zeros, isnan, isinf
...
def full(
    shape: tuple[int, ...], fill_value: float, dtype: Any = np.float32, device: str | None = None
) -> np.ndarray:
    """Create constant-filled array — replaces torch.full."""
    return np.full(shape, fill_value, dtype=dtype)


def full_like(a: np.ndarray, fill_value: float, dtype: Any | None = None) -> np.ndarray:
...
def randn(*shape, dtype: Any = np.float32, device: str | None = None) -> np.ndarray:
...
def clamp(
    a: Any, min=None, max=None, min_val: float | None = None, max_val: float | None = None
) -> np.ndarray:
...
def softmax(a: Any, dim: int = -1) -> np.ndarray:
    ...
    return (exp_arr / np.sum(exp_arr, axis=dim, keepdims=True)).astype(
