...
class HealthWatchdog:
    ...
    def __init__(self) -> None:
        ...
        self._recovery_fn: Callable | None = None
        self._health_check_fn: Callable[[], bool] | None = None
    ...
    def start(self, poll_interval: int = 10, max_failures: int = 3) -> None:
        ...
        self._thread.start()
        logger.info(
            "Health watchdog started (interval=%ds, max_failures=%d)",
            poll_interval,
            max_failures,
            extra={"tag": "INFRA"},
        )
    ...
    def stop(self) -> None:
        ...
        logger.info("Health watchdog stopped", extra={"tag": "INFRA"})
                ...
                with self._lock:
                    if healthy:
                        ...
                        if self._consecutive_failures >= self._max_failures:
                            logger.error(
                                "Server unhealthy — triggering recovery", extra={"tag": "INFRA"}
                            )
                            self._consecutive_failures = 0
                            self._trigger_recovery()

            except Exception as e:
                logger.error("Watchdog error: %s", e, extra={"tag": "INFRA"})
    ...
    def _trigger_recovery(self):
        ...
        if not self._recovery_fn:
            logger.warning("No recovery function set — cannot recover", extra={"tag": "INFRA"})
            return

        try:
            logger.info("Running recovery procedure...", extra={"tag": "INFRA"})
            result = self._recovery_fn()
            if result:
                logger.info("Recovery successful", extra={"tag": "INFRA"})
                if self._on_recovery:
                    self._on_recovery()
            else:
                logger.error("Recovery failed", extra={"tag": "INFRA"})
        except Exception as e:
            logger.error("Recovery procedure crashed: %s", e, extra={"tag": "INFRA"})
