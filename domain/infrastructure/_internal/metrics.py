    ...
    def render(self) -> str:
        ...
        with self._lock:
            ...
            for path, latencies in sorted(self._request_latencies.items()):
                ...
                sum(sorted_lat) / n
                lines.append(
                    f'sloughgpt_request_duration_seconds{{path="{path}",quantile="0.5"}} {p50:.4f}'
                )
                lines.append(
                    f'sloughgpt_request_duration_seconds{{path="{path}",quantile="0.95"}} {p95:.4f}'
                )
                lines.append(
                    f'sloughgpt_request_duration_seconds{{path="{path}",quantile="0.99"}} {p99:.4f}'
                )
                lines.append(
                    f'sloughgpt_request_duration_seconds_sum{{path="{path}"}} {sum(sorted_lat):.4f}'
                )
            ...
            if self._inference_latencies:
                ...
                lines.append(
                    "# HELP sloughgpt_inference_duration_seconds Inference latency percentiles."
                )
            ...
            if self._startup_hook_durations:
                lines.append(
                    "# HELP sloughgpt_startup_hook_duration_seconds Startup hook duration."
                )
                ...
                for hook, duration in sorted(self._startup_hook_durations.items()):
                    lines.append(
                        f'sloughgpt_startup_hook_duration_seconds{{hook="{hook}"}} {duration:.2f}'
                    )

            # Startup stage durations
            if self._startup_stage_durations:
                lines.append(
                    "# HELP sloughgpt_startup_stage_duration_seconds Startup stage duration."
                )
                ...
                for stage, duration in sorted(self._startup_stage_durations.items()):
                    lines.append(
                        f'sloughgpt_startup_stage_duration_seconds{{stage="{stage}"}} {duration:.2f}'
                    )
            ...
            lines.append(
                "# HELP sloughgpt_startup_regression_count Total startup regressions detected."
            )
