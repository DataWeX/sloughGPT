    ...
    * ``minimal`` — only core infrastructure (logging, registry, routers)
    ...
    def from_env(cls) -> Self:
        ...
        try:
            ...
            logger.warning(
                "Unknown startup profile %r — falling back to full", raw, extra={"tag": "INFRA"}
            )
            return cls.FULL
...
def _dependency_levels(hooks: list[StartupHook]) -> list[list[StartupHook]]:
    ...
    while remaining:
        ...
        level = [
            h for h in remaining if all(d in resolved or d not in by_name for d in h.depends_on)
        ]
        if not level:
            # Break cycle: put remaining in one level
            level = list(remaining)
            logger.warning(
                "Dependency cycle in parallel startup — running remaining hooks sequentially",
                extra={"tag": "INFRA"},
            )
        levels.append(level)
        for h in level:
...
class LifecycleManager:
    ...
    async def _set_phase(self, phase: LifecyclePhase) -> None:
        ...
        old = self._phase
        self._phase = phase
        logger.info("Lifecycle: %s → %s", old.value, phase.value, extra={"tag": "INFRA"})
    ...
    def register_startup_hook(self, hook: StartupHook) -> None:
        ...
        if any(h.name == hook.name for h in self._startup_hooks):
            logger.warning(
                "Duplicate startup hook: %s — skipping", hook.name, extra={"tag": "INFRA"}
            )
            return
    ...
    def register_shutdown_hook(self, hook: ShutdownHook) -> None:
        ...
        if any(h.name == hook.name for h in self._shutdown_hooks):
            logger.warning(
                "Duplicate shutdown hook: %s — skipping", hook.name, extra={"tag": "INFRA"}
            )
            return
    ...
    async def wait_for_gates(self, timeout: float = 30.0, poll_interval: float = 0.5) -> bool:
        ...
        logger.warning("Health gates not ready after %.0fs", timeout, extra={"tag": "INFRA"})
        return False
    ...
    async def _wait_for_drain(self, timeout: float = 30.0) -> bool:
        ...
        if self._in_flight_count == 0:
            return True
        logger.info(
            "Draining: waiting for %d in-flight tasks",
    ...
    async def start(self, timeout: float = 60.0, profile: StartupProfile | None = None) -> bool:
        ...
        async with self._lock:
            if self._phase != LifecyclePhase.INIT:
                logger.warning(
                    "start() called in phase %s — ignoring",
                    self._phase.value,
                    extra={"tag": "INFRA"},
                )
                return self._phase == LifecyclePhase.RUNNING
        ...
        for level in levels:
            if len(level) == 1:
                ...
                try:
                    ...
                    self._emit_sync(
                        EVT_HOOK_COMPLETED, {"hook": hook.name, "elapsed": round(elapsed, 3)}
                    )
                    logger.info(
                        "Startup hook %s completed in %.2fs",
                        hook.name,
                        elapsed,
                        extra={"tag": "INFRA"},
                    )
                    ...
                    results.append(
                        _HookResult(name=hook.name, success=False, elapsed=elapsed, error=error)
                    )
                    self._emit_sync(
                        EVT_HOOK_FAILED,
                        {"hook": hook.name, "error": error, "elapsed": round(elapsed, 3)},
                    )
                    logger.error("Startup hook %s %s", hook.name, error, extra={"tag": "INFRA"})
                    if hook.critical:
                        all_ok = False
                        break
                    ...
                    logger.error(
                        "Startup hook %s failed: %s", hook.name, exc, extra={"tag": "INFRA"}
                    )
                    if hook.critical:
                        all_ok = False
                        break
                ...
                logger.info(
                    "Startup: running %d hooks in parallel: %s",
                    len(level),
                    ", ".join(names),
                    extra={"tag": "INFRA"},
                )
                ...
                async def _run_hook(h: StartupHook) -> _HookResult:
                    ...
                    try:
                        ...
                        elapsed = time.time() - started
                        self._emit_sync(
                            EVT_HOOK_COMPLETED, {"hook": h.name, "elapsed": round(elapsed, 3)}
                        )
                        logger.info(
                            "Startup hook %s completed in %.2fs",
                            h.name,
                        ...
                        self._emit_sync(
                            EVT_HOOK_FAILED,
                            {"hook": h.name, "error": error, "elapsed": round(elapsed, 3)},
                        )
                        ...
                        logger.error(
                            "Startup hook %s failed: %s", h.name, exc, extra={"tag": "INFRA"}
                        )
                        return _HookResult(name=h.name, success=False, elapsed=elapsed, error=error)
                ...
                logger.info(
                    "Startup: parallel level (%s) completed in %.2fs",
                    ", ".join(names),
                    level_elapsed,
                    extra={"tag": "INFRA"},
                )
        ...
        async with self._lock:
            if all_ok:
                ...
                self._emit_sync(
                    "lifecycle.started", {"elapsed": round(time.time() - self._started_at, 3)}
                )
                return True
    ...
    async def shutdown(self, timeout: float = 60.0) -> bool:
        ...
        async with self._lock:
            if self._phase not in (LifecyclePhase.RUNNING, LifecyclePhase.CRASHED):
                logger.warning(
                    "shutdown() called in phase %s — ignoring",
                    self._phase.value,
                    extra={"tag": "INFRA"},
                )
        ...
        shutdown_list = _topological_sort(
            [
                StartupHook(
                    name=h.name, handler=h.handler, depends_on=h.depends_on, timeout=h.timeout
                )
    ...
    def get_results(self) -> dict[str, Any]:
        ...
        return {
            ...
            "gates": {
                ...
                "passed": sum(1 for n in self._health_gates if self._gate_results.get(n, False)),
            },
        }
