...
class ProducerConsumerQueue[T]:
        ...
        try:
            if self._use_priority:
                with self._seq_lock:
                    ...
                    seq = self._seq
                self._queue.put(
                    _PriorityItem(priority=priority, sequence=seq, item=item),
                    timeout=timeout,
                    block=True,
                )
            else:
    ...
    async def async_put(self, item: T, timeout: float | None = None, priority: int = 0) -> bool:
        ...
        return await loop.run_in_executor(
            None, lambda: self.put(item, timeout=timeout, priority=priority)
        )
    ...
    def start(self) -> None:
        ...
        for i in range(self.num_consumers):
            ...
            t.start()
        logger.info(
            "ProducerConsumerQueue[%s] started (%d consumers, maxsize=%d, priority=%s)",
            self.name,
            self.num_consumers,
    ...
    def stop(self, timeout: float = 5.0) -> None:
            ...
            try:
                if self._use_priority:
                    ...
                    self._queue.put_nowait(
                        _PriorityItem(priority=999999, sequence=self._seq, item=_POISON_PILL)
                    )
                else:
        ...
        with self._metrics_lock:
            ...
            if self._shutdown_mode == ShutdownMode.DROP and queued > 0:
                ...
                logger.info(
                    "ProducerConsumerQueue[%s] dropped %d items on shutdown", self.name, queued
                )
            else:
                logger.info(
                    "ProducerConsumerQueue[%s] stopped, %d items remaining", self.name, queued
                )
            ...
            while not self._stop_event.is_set():
                ...
                try:
                    ...
                    with self._metrics_lock:
                        self._errors += 1
                    logger.warning(
                        "Consumer %d handler error: %s", consumer_id, e, extra={"tag": "INFRA"}
                    )
                finally:
                    self.task_done()
        finally:
    ...
    def __repr__(self) -> str:
        return (
            f"ProducerConsumerQueue(name={self.name!r}, consumers={self.num_consumers}, "
