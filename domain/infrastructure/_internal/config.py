    ...
    for dot_path, value in overrides.items():
        ...
        for part in parts[:-1]:
            if part not in target:
                logger.warning(
                    "Unknown config key: %s (from env %s)",
                    dot_path,
                    _ENV_PREFIX + dot_path.upper().replace(".", _SEPARATOR),
                    extra={"tag": "INFRA"},
                )
                break
            target = target[part]
...
def _load_yaml(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning("Failed to load config file: %s", path, extra={"tag": "INFRA"})
    return {}
...
class ConfigManager:
    ...
    def __init__(self, config_dir: str | Path | None = None):
        ...
        try:
            ...
            logger.warning(
                "Could not create config directory: %s", self._config_dir, extra={"tag": "INFRA"}
            )
    ...
    def _write_example_config(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Could not write example config: %s", example_path, extra={"tag": "INFRA"}
            )
    ...
    def reload(self) -> AppConfig:
        ...
        try:
            ...
            logger.warning("Could not emit config.changed event", extra={"tag": "INFRA"})
