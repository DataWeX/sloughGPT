...
class WorkerPool:
    ...
    async def start(self):
        ...
        logger.info("Worker pool started with %d workers", self.num_workers, extra={"tag": "INFRA"})
    ...
    async def stop(self, timeout: float = 5.0):
        ...
        if pending_workers:
            ...
            for p in pending:
                p.cancel()
        self._workers.clear()
        logger.info("Worker pool stopped", extra={"tag": "INFRA"})
    ...
    async def _worker_loop(self, worker_id: int):
        while self._running:
            ...
            try:
                ...
                logger.exception(
                    "Worker %d handler failed for task %s",
                    worker_id,
                    task.id,
                    extra={"tag": "INFRA"},
                )
...
class TaskQueue:
    ...
    async def start(self):
        ...
        logger.info(
            "TaskQueue started with %d workers", self._pool.num_workers, extra={"tag": "INFRA"}
        )
    ...
    async def stop(self, timeout: float = 5.0):
        ...
        logger.info("TaskQueue stopped", extra={"tag": "INFRA"})
