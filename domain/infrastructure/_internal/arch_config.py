...
def build_arch(name: str, config: dict, weight_keys: set) -> ArchConfig:
    ...
    if "wte.weight" in weight_keys:
        ...
        norm = (
            "rms_norm" if "model.layers.0.input_layernorm.weight" in weight_keys else "layer_norm"
        )
        positional = "rope"
        ...
        wm = LLAMA_WEIGHT_MAP
        transpose = False  # LLaMA safetensors stores (out, in), same as SloLinear
    else:
    ...
    logger.info(
        "ArchConfig: %s (norm=%s, pos=%s, act=%s, attn=%s, layers=%d, heads=%d/%d)",
        name,
        norm,
        positional,
        activation,
