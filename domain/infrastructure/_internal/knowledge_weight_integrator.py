...
def _resolve_model(model, vocab_size: int):
    ...
    try:
        ...
        return SloughGPTModel(
            ...
            max_seq_len=_BLOCK_SIZE,
        )
    except Exception as e:
        logger.warning(
            "Could not construct a native model for training: %s", e, extra={"tag": "INFRA"}
        )
        return None
...
def _save_delta(model, stoi: dict[str, int], itos: dict[int, str], rank: int) -> None:
    ...
    arrays["_stoi"] = np.frombuffer(json.dumps(stoi).encode("utf-8"), dtype=np.uint8)
...
def train_knowledge_adapter(
    ...
    logger.info(
        "Knowledge adapter trained in %.1fs on %d facts (loss %.4f)",
        elapsed,
        len(texts),
        final_loss,
        extra={"tag": "INFRA"},
...
def load_knowledge_adapter(
    ...
    if not merge:
        logger.info(
            "Knowledge adapter loaded (unmerged; use merge=True to apply)", extra={"tag": "INFRA"}
        )
        return model
    ...
    for name, param in model.named_parameters():
        ...
        if current.shape == delta.shape:
            ...
            applied += 1
    data.close()
    logger.info("Knowledge adapter merged into model (%d tensors)", applied, extra={"tag": "INFRA"})
    return model
...
def get_adapter_status() -> dict[str, Any]:
    ...
    adapter_exists = _DELTA_PATH.exists() or (_ADAPTER_PATH / "adapter_config.json").exists()
    return {
