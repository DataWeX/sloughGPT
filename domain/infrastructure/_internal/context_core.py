...
Be concise, accurate, and helpful."""
    ...
    def __init__(
        ...
        consciousness_manager: Any | None = None,
    ):
    ...
    def _auto_ingest(self) -> None:
        ...
        def _do():
            try:
                ...
                from .auto_ingest import AutoIngester

                ingester = AutoIngester(provider=provider)
                asyncio.run(ingester.ingest())
            except Exception as e:
                logger.warning(
                    "context_core: auto-ingestion failed",
                    extra={
                        "provider": provider,
                        "error": str(e),
                    },
    ...
    async def get_rag_context(self, query: str) -> str:
        ...
        if self._vector_store is None:
            ...
            try:
                ...
                def _query_aug():
                    result = enrich_with_knowledge(
                        query, auto_search=False, max_facts=self.rag_top_k
                    )
                ...
                logger.debug(
                    "context_core: knowledge augmenter query failed",
                    extra={
                        "error": str(e),
                    },
                )
    ...
    async def build_context_frame(
        ...
        if include_memory and self.memory_enabled:
            ...
            auto_memory = await self.get_auto_memory_context(query, limit=self._AUTO_MEMORY_TOP_K)
            if auto_memory:
                memory_content = (
                    f"{memory_content}\n{auto_memory}".strip() if memory_content else auto_memory
                )
            if memory_content:
                layer = ContextLayer(
                    layer_type="memory",
...
# Global instance
_context_core: ContextCore | None = None
                ...
                if vs_provider:
                    try:
                        ...
                        if vs_provider:
                            ...
                            logger.info(
                                "Vector store auto-configured: %s",
                                vs_provider,
                                extra={"tag": "INFRA"},
                            )
                    except Exception as e:
                        logger.warning(
                            "Failed to auto-configure vector store %s: %s",
                            vs_provider,
                            e,
                            extra={"tag": "INFRA"},
                        )
