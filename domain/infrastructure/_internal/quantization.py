...
Usage:
    from .quantization import Quantine, TensorInfo
...
class TensorInfo:
    ...
    def as_float(self) -> np.ndarray:
        ...
        signed = (
            (self.meta.mode == "symmetric") if self.is_quantized and self.meta.bits == 4 else True
        )
        return _dequantize(
            self.array,
...
class Quantine:
    ...
    def __init__(
        ...
        logger.info(
            ...
            bits,
            mode,
            clip_percentile or 0.0,
            skip_quantize_if_error_above,
            extra={"tag": "INFRA"},
        )
    ...
    def quantize(self, name: str, arr: np.ndarray) -> TensorInfo:
        ...
        dequantized = _dequantize(
            quantized, scale, zero_point, self._bits, arr.shape, signed=_signed
        )
    ...
    def _quantize_per_channel(self, name: str, arr: np.ndarray) -> TensorInfo:
        ...
        if self._bits == 4:
            ...
            packed = (quantized[:, 1::2].astype(np.uint8) << 4) | (
    ...
    def save_metadata(self, path: str):
        ...
        logger.info(
            "Quantine: saved metadata for %d tensors to %s",
            len(report),
            path,
            extra={"tag": "INFRA"},
        )
    ...
    def load_metadata(self, path: str):
        ...
        logger.info(
            "Quantine: loaded metadata for %d tensors from %s",
            len(self._error_report),
            path,
            extra={"tag": "INFRA"},
        )
    ...
    def save_weights(self, path: str, tensor_infos: dict[str, TensorInfo]):
        ...
        logger.info(
            ...
            sum(1 for k in arrays if not k.startswith("_meta_")),
            path,
            extra={"tag": "INFRA"},
        )
    ...
    def load_weights(self, path: str) -> dict[str, TensorInfo]:
        ...
        for name in sorted(names):
            ...
            if meta_key not in data:
                logger.warning(
                    "Quantine: missing metadata for %s, skipping", name, extra={"tag": "INFRA"}
                )
                continue
    ...
    def suggest_format(
        ...
        for fmt in ("int8", "int4"):
            r = results[fmt]
            if (
                r["cosine_sim"] >= quality_threshold
                and r["throughput"] >= best_tp * min_speed_ratio
            ):
...
def int4_matmul(
    ...
    if a_zero_point == 0 and b_zero_point == 0:
        ...
        N = b_packed.shape[0]
        b_unpacked = _unpack_int4(b_packed.ravel(), N * orig_k, signed=True).reshape(N, orig_k)
...
def int8_matmul(
    ...
    if a_zero_point == 0 and b_zero_point == 0:
        ...
        accum = (
            accum
            - a_zero_point * b_sum.T
            - b_zero_point * a_sum
            + a.shape[-1] * a_zero_point * b_zero_point
        )
        return accum.astype(np.float32) * (a_scale * b_scale)
...
def quantized_linear(
    ...
    result = int8_matmul(
        ...
        a_zero_point=x_zero_point,
        b_zero_point=weight_zero_point,
    )
...
def int4_quantized_linear(
    ...
    orig_k: int,
    bias: np.ndarray | None = None,
    x_scale: float | np.ndarray | None = None,
) -> np.ndarray:
...
class QuantizedLinear:
    ...
    def __init__(
        self, weight_int8, scale, zero_point, bias, bits, original_shape, mode="symmetric"
    ):
