...
from .compute_backend import ComputeBackend, register_backend
...
def _matmul_worker(args):
    ...
    shm_name_a, shape_a, dtype_a, start, end, shm_name_b, shape_b, dtype_b, out_shm, out_shape = (
        args
    )
...
class VectorBE(ComputeBackend):
    ...
    def softmax(self, x: np.ndarray, axis: int = -1) -> np.ndarray:
        if x.ndim == 2 and axis == -1:
            ...
            for start in range(0, rows, chunk):
                ...
                tasks.append((x_shm.name, x.shape, x.dtype, start, end, out_shm.name, (rows, cols)))
    ...
    def repeat_kv(self, x: np.ndarray, n_reps: int) -> np.ndarray:
        if n_reps == 1:
            return x
        b, h, seq, d = x.shape
        return np.broadcast_to(x[:, :, None, :, :], (b, h, n_reps, seq, d)).reshape(
            b, h * n_reps, seq, d
        )
