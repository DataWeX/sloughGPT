...
class ModelRegistry:
    ...
    def __init__(self) -> None:
        self._servers: dict[str, ModelServer | Any] = {}
        self._lock = Lock()
    ...
    def _emit_event(self, event: str, model_id: str, **extra: Any) -> None:
        try:
            ...
            logger.warning(
                "EventBus emit failed for %s/%s: %s", event, model_id, exc, extra={"tag": "INFRA"}
            )
    ...
    def register(
        ...
        logger.info(
            ...
            server._device,
            generate_timeout,
            max_concurrent if max_concurrent is not None else "default",
            extra={"tag": "MODEL"},
        )
        self._emit_event(
            "model.registered",
    ...
    def get(self, model_id: str | None = None) -> ModelServer | None:
        ...
        with self._lock:
            ...
            if server is None:
                logger.debug(
                    "model_registry: model '%s' not found (available: %s)",
                    mid,
    ...
    def health_summary(self) -> dict:
        ...
        loaded_count = sum(1 for m in models if m.get("status") in ("ready", "degraded"))
