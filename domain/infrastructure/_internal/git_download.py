...
Usage::
    ...
    from .git_download import GitBackend
...
class GitBackend(DownloadBackend):
    ...
    def __init__(self, *, ssh_key: str | None = None, token: str | None = None):
        self._ssh_key = ssh_key
        self._token = token

    def _parse_resource(self, resource_id: str) -> tuple[str, str, str | None]:
        ...
        if len(parts) < 2:
            raise ValueError(
                f"Invalid git resource format: {resource_id!r}. Expected 'org/repo[@branch]'."
            )
        org = parts[0]
        repo = parts[1]
