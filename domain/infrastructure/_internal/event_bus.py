...
EventHandler = Callable[..., Awaitable[Any] | Any]
    ...
    def emit_sync(
        ...
        for target, sub in subs:
            ...
            try:
                ...
                if asyncio.iscoroutine(result):
                    logger.warning(
                        "event_bus: async handler %s ignored by emit_sync (coroutine not awaited)",
                        getattr(sub.handler, "__name__", "?"),
        ...
        if handler:
            for evt in past:
                try:
                    ...
                    if asyncio.iscoroutine(result):
                        warnings.warn("async handler passed to sync replay()", stacklevel=2)
                except Exception:
                    logger.exception(
                        "Replay handler failed for %s", evt.name, extra={"tag": "INFRA"}
                    )
        return past
