...
class AutoIngester:
    ...
    async def get_vector_store(self):
        ...
        try:
            ...
            logger.warning(
                "Vector store not available. Running in dry-run mode.", extra={"tag": "INFRA"}
            )
            return None
    ...
    async def ingest(self, dry_run: bool = False) -> dict[str, Any]:
        ...
        # Scan and chunk
        logger.info("Scanning %s...", self.root, extra={"tag": "INFRA"})
        ...
        if dry_run or not store:
            logger.info("  Dry run — not writing to vector store", extra={"tag": "INFRA"})
            return self.stats

        # Upsert to vector store
        logger.info("  Ingesting to %s...", self.provider, extra={"tag": "INFRA"})
        ...
        logger.info("  %d chunks ingested", count, extra={"tag": "INFRA"})
        return self.stats
...
async def main():
    ...
    parser.add_argument(
        "--provider", default="chromadb", choices=["chromadb", "in_memory", "pinecone"]
    )
