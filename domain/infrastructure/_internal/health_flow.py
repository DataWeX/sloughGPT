...
def _check_errors(req_count: int, err_count: int) -> Diagnosis:
    ...
    severity = Severity.OK if score >= 80 else Severity.WARN if score >= 50 else Severity.CRITICAL
...
def _check_resources(cpu_percent: float, memory_percent: float) -> Diagnosis:
    ...
    severity = Severity.OK if score >= 70 else Severity.WARN if score >= 40 else Severity.CRITICAL
