...
async def training_handler(task) -> dict:
    ...
    if enqueue is None:
        logger.error(
            "training_handler: no enqueue callback in task metadata",
            extra={
                "task_id": task.id,
    ...
    try:
        ...
        if cancel_event.is_set():
            ...
            enqueue(
                sse_complete("auto-train", data={"cancelled": True}, message="Training cancelled")
            )
...
def _resolve_checkpoint(name: str | None, checkpoint_dir: str) -> str | None:
    ...
    logger.warning("Checkpoint not found: %s in %s", name, checkpoint_dir, extra={"tag": "TRAIN"})
    return None
...
async def training_sessions_handler(task) -> dict:
    ...
    if enqueue is None:
        logger.error(
            "training_sessions_handler: no enqueue callback in task metadata",
            extra={
                "task_id": task.id,
                "task_type": task.task_type,
                "metadata_keys": list(task.metadata.keys()),
