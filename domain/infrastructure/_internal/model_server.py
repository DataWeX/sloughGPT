...
from .circuit_breaker import CircuitBreaker, CircuitBreakerState  # noqa: F401
from .constants import DEFAULT_GENERATE_TIMEOUT
from .idle_manager import IdleManager, get_idle_manager  # noqa: F401
from .model_status import ModelMetrics, ModelStatus  # noqa: F401

# Re-export extracted modules for backward compatibility
from .request_queue import (  # noqa: F401
    Priority,
...
def _emit_gen_event(event: str, data: dict) -> None:
    ...
    try:
        ...
        logger.debug(
            "EventBus emit failed: %s", exc
        )  # EventBus unavailable or disabled — generation still works
...
def _has_mps() -> bool:
    try:
        from .ml_types import mps as ml_mps
...
class GuardBackend(GenerateBackend):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        safe_kwargs = {k: v for k, v in kwargs.items() if k not in ("input_ids", "attention_mask")}
        start = time.time()
...
class _TokenStreamer:
    ...
    def __init__(
        self, tokenizer, skip_prompt: bool = False, timeout: float = DEFAULT_GENERATE_TIMEOUT
    ):
        ...
        **kwargs: Any,
        ...
        if session_id is not None and self._model_ref is not None:
            if pkv is not None:
                ...
                try:
                    captured = getattr(self._model_ref, "_past_key_values", None) or getattr(
    ...
    def generate_stream(
        ...
        **kwargs: Any,
        ...
        streamer = _TokenStreamer(
            self._tokenizer, skip_prompt=True, timeout=DEFAULT_GENERATE_TIMEOUT
        )
...
class ModelServer:
    ...
    def __init__(
        ...
        self._guard_backend: GuardBackend | None = (
        ...
        self._request_queue: PriorityRequestQueue | None = None
        ...
        if self._guard_backend is not None and self._circuit_breaker is not None:
            self._process_guard.on_crash(lambda wid: self._circuit_breaker.record_failure())
            self._process_guard.on_restart(lambda wid: self._circuit_breaker.record_success())
        ...
        self._hf_model_id: str | None = None  # stored for idle reload
        self._slnc_path: str | None = None  # stored for idle reload
    ...
    def _idle_unload(self) -> None:
        ...
        gc.collect()
        logger.info(
            "Model %s unloaded (idle) — memory freed",
            self.model_id,
            extra={"tag": "IDLE"},
        )
    ...
    def _idle_reload(self) -> None:
        ...
        if not self._hf_model_id and not self._slnc_path:
            logger.warning(
                "Cannot reload %s: no model ID or path stored",
                self.model_id,
                extra={"tag": "IDLE"},
            )
            return
        try:
            ...
            if not slnc_path:
                ...
                from .model_resolver import get_model_dir as _get_model_dir
            ...
            with self._lock:
                ...
                self._status = ModelStatus.READY
            logger.info(
                "Model %s reloaded (idle) — ready to serve",
                self.model_id,
                extra={"tag": "IDLE"},
            )
        except Exception as e:
    ...
    def _run_warmup(self) -> None:
        ...
        try:
            ...
            logger.info(
                "ModelServer[%s]: warmup completed (%dms)",
                self.model_id,
                int(elapsed_ms),
                extra={"op": "model.warmup", "model": {"id": self.model_id}},
            )
        except Exception as e:
            with self._warmup_lock:
            ...
            if "No module named" in str(e):
                ...
                logger.warning(
                    "ModelServer[%s]: warmup failed: %s", self.model_id, e, extra={"tag": "MODEL"}
                )
            return
    ...
    def _check_device(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "model_server: device detection failed",
                extra={
    ...
    def drop_model_ref(self) -> None:
        ...
        self._device = "guard"
        logger.info(
            "ModelServer[%s]: dropped in-memory model ref (guard mode)",
            self.model_id,
            extra={"op": "model.unload", "model": {"id": self.model_id}},
        )
    ...
    def _on_cb_state_change(self, old: CircuitBreakerState, new: CircuitBreakerState) -> None:
        try:
            ...
            logger.warning(
                "model_server: circuit breaker state change event failed",
                extra={
                    "model_id": self.model_id,
                    "old_state": old.value,
                    "new_state": new.value,
    ...
    async def _run_queue_workers(self, q: PriorityRequestQueue) -> None:
        ...
        try:
            ...
            logger.error(
                "model_server: queue worker pool crashed",
                extra={
                    "model_id": self.model_id,
                    "n_workers": n_workers,
                    "error": str(e),
    ...
    async def _get_read_semaphore(self) -> asyncio.Semaphore | None:
    ...
    async def tokenize(self, text: str) -> dict:
        ...
        try:
            ...
            loop = asyncio.get_running_loop()
            return await loop.run_in_executor(None, lambda: tok(text, return_tensors="pt"))
        finally:
        ...
        **kwargs: Any,
        ...
        if self._idle_timeout_s > 0:
            ...
            if idle_status == "reloading":
                ...
                raise RuntimeError(f"Model {self.model_id} failed to reload after idle timeout.")
        ...
        async def _run() -> dict:
            ...
            try:
                ...
                logger.warning(
                    "Generation timed out", model_id=self.model_id, timeout=self._generate_timeout
                )
                _emit_gen_event(
                    "generation.failed",
                    {
        ...
        try:
            return await queue.submit(
                _run(), priority=priority, request_id=f"gen-{id(prompt[:32])}"
            )
    ...
    async def generate_stream(
        ...
        **kwargs: Any,
        ...
        is_local = isinstance(backend, LocalBackend)
        logger.debug(
            "generate_stream[%s]: backend=%s session_id=%s",
            self.model_id,
        ...
        try:
            ...
            aborted = True
            logger.info(
                "generate_stream[%s]: client disconnected mid-stream",
                self.model_id,
                extra={"tag": "MODEL"},
            )
            if cancel_event is not None:
            ...
            if pump_thread is not None and pump_thread.is_alive():
                ...
                if pump_thread.is_alive():
                    logger.warning(
                        "generate_stream[%s]: pump thread did not stop within 10s",
                        self.model_id,
                        extra={"tag": "MODEL"},
                    )
            return
            ...
            if aborted:
                logger.info(
                    "generate_stream[%s]: cleaned up after abort",
                    self.model_id,
                    extra={"tag": "MODEL"},
                )
