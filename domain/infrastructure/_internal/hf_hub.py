...
def _model_cache_candidates(model_id: str, hf_home: str | None) -> list[Path]:
...
def find_cached_model_dir(model_id: str, hf_home: str | None = None) -> Path | None:
...
def _has_incomplete_downloads(cache_dir: Path) -> bool:
    """True when a HF cache entry has in-progress or interrupted markers."""
    if _has_incomplete_markers(cache_dir):
        return True
    return bool(list(cache_dir.rglob("*.lock")))


def _has_complete_snapshot(cache_dir: Path) -> bool:
    """Whether a HF cache entry has a complete snapshot layout."""
    snapshot = _snapshot_dir(cache_dir)
    if snapshot is None:
        return False
    return _has_weight_files(snapshot) and not _has_incomplete_downloads(cache_dir)
...
def _derive_model_id(path: str | Path) -> str | None:
...
from .download_backend import DownloadBackend, FileEstimate
...
class HFDownloadBackend(DownloadBackend):
    ...
    def serve_compressed(self, resource_id: str, file_path: str) -> dict | None:
        ...
        """
        from .compressed_transfer import CompressedFileServer
