...
if TYPE_CHECKING:
    from .point_compressor import ModelTree, PointLibrary
...
class NumpyEngine:
    ...
    def __init__(
        ...
        n_clusters: int = 16,
        cache_size: int = 100,
        model_tree: ModelTree | None = None,
    ):
        ...
        self._model_tree: ModelTree | None = model_tree
    ...
    def from_slnc(
        ...
        logger.info(
            "NumpyEngine.from_slnc: %s, %d layers",
            slnc_path,
            config.get("n_layer", 0),
            extra={"tag": "INFRA"},
        )
    ...
    def _forward(
        self, token_ids: list[int], kv_cache: KVCache | None = None, start_pos: int = 0
    ) -> np.ndarray:
    ...
    def info(self) -> dict[str, Any]:
        ...
        return {
            "arch": self.arch.name,
            "arch_config": self.arch.norm
            + "/"
            + self.arch.positional
            + "/"
            + self.arch.activation
            + "/"
            + self.arch.attention,
            "vocab_size": self.vocab_size,
            "max_context": self.max_context,
            "num_layers": self.arch.n_layers,
            "num_params": sum(w.size for w in self._raw_weights.values())
            + sum(c.shape[0] for c in self._compressed_weights.values()),
