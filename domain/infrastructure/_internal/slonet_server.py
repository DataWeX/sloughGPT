...
class SloNetServer:
    ...
    def __init__(
        ...
        lazy_model_factory: Callable[[], Any] | None = None,
    ):
        ...
        if process_guard is not None and self._circuit_breaker is not None:
            process_guard.on_crash(lambda w: self._circuit_breaker.record_failure())
            process_guard.on_restart(lambda w: self._circuit_breaker.record_success())
            ...
            if self._lazy_model_factory is not None:
                with self._lazy_lock:
                    if self._model is None:
                        ...
                        self._model = model
                        logger.info(
                            "slonet_server: lazy model materialized",
                            extra={
                    ...
                    return self._model
            raise RuntimeError(f"SloNetServer '{self._model_id}' has no model and no lazy factory")
        try:
    ...
    def _guard_health(self) -> dict | None:
        ...
        try:
            ...
            logger.warning(
                "slonet_server: guard health check failed",
                extra={
                    "model_id": self._model_id,
                    "error": str(e),
                },
    ...
    def _generate_stream_sync(
        ...
        model = self._acquire_model()
        logger.debug(
            "PUMP_STREAM: _acquire_model returned in %.1fms, model=%s",
        ...
        try:
            ...
            tokens = self._tokenizer.encode(prompt)
            logger.debug(
                "PUMP_STREAM: encode done in %.1fms, tokens=%d",
                (_time.monotonic() - _t_enc) * 1000,
                len(tokens),
            )
            input_ids = np.array([tokens], dtype=np.int64)
            ...
            kv_state = self._resolve_kv_state(session_id)
            logger.debug(
                "PUMP_STREAM: kv_state resolved in %.1fms, session=%s",
                (_time.monotonic() - _t_kv) * 1000,
                session_id,
            )
            ...
            with getattr(self, "_model_lock", contextlib.nullcontext()):
                for tok_id in model.generate_numpy_stream(
                    ...
                    if _first_token:
                        logger.debug(
                            "PUMP_STREAM: first token from generate_numpy_stream in %.1fms",
                    ...
                    if decoded:
                        yield decoded
            logger.debug(
                "PUMP_STREAM: generation complete in %.1fms", (_time.monotonic() - _t_gen) * 1000
            )
        finally:
            self._release_model(model)
    ...
    def metadata(self) -> dict:
        ...
        if m is None and self._lazy_model_factory is not None and self._provider is not None:
            try:
                ...
                logger.warning(
                    "slonet_server: provider metadata parse failed",
                    extra={
                        "model_id": self._model_id,
                        "error": str(e),
                    },
        ...
        cb_state = self._circuit_breaker.state.value if self._circuit_breaker else "disabled"
        with self._warmup_lock:
