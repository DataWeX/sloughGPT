...
Usage::
    ...
    from .cancel_manager import get_cancel_manager, OpType
...
def _emit_op_event(action: str, op: Operation) -> None:
    ...
    try:
        ...
        logger.debug(
            "EventBus emit failed in CancelManager: %s", exc
        )  # Never let event emission break cancellation logic
...
class CancelManager:
    ...
    def list_all(self, op_type: OpType | None = None, limit: int = 200) -> list[Operation]:
        with self._lock:
            ops = [op for op in self._ops.values() if op_type is None or op.op_type == op_type]
            return ops[-limit:]
