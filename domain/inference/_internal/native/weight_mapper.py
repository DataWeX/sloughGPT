...
def map_slnc_to_native(
    ...
    def prefix_fn(i):
        return f"model.layers.{i}" if is_qwen_style else f"h.{i}"
    ...
    flat[offset : offset + D] = fnw_f.astype(np.float32).ravel()[:D]
    offset += D
    ...
    flat[offset : offset + V * D] = lm_head.astype(np.float32).ravel()[: V * D]
    offset += V * D
    ...
    logger.info("weight_mapper: %d floats (%.1f MB), %d layers", offset, offset * 4 / 1e6, n_layers)
