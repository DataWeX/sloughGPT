...
class ToolRegistry:
    ...
    def _register_defaults(self) -> None:
            ...
            ToolSpec(
                ...
                parameters=[
                    ToolParam(
                        "expression",
                        "string",
                        "The mathematical expression to evaluate, e.g. 2+2 or sqrt(144)",
                        required=True,
                    ),
                ],
                execute=self._run_calculator,
                pattern=re.compile(
                    r"^(?:calculate|calc|math|what is|what's)\s+(.+)$", re.IGNORECASE
                ),
            )
        )
        self.register(
            ToolSpec(
                ...
                parameters=[
                    ToolParam(
                        "timezone", "string", "Timezone (optional, default local)", required=False
                    ),
                ],
                execute=self._run_current_time,
                pattern=re.compile(
                    r"^(?:time|date|what time|what date|what's the time|current time)\s*$",
                    re.IGNORECASE,
                ),
            )
        )
        self.register(
            ToolSpec(
                ...
                pattern=re.compile(
                    r"^(?:search|look up|find|google|web search)\s+(.+)$", re.IGNORECASE
                ),
        ...
        self.register(
            ToolSpec(
                ...
                pattern=re.compile(
                    r"^(?:read|open|show|cat|view)\s+(?:file\s+)?(.+)$", re.IGNORECASE
                ),
            )
        )
        self.register(
            ToolSpec(
                ...
                parameters=[
                    ToolParam(
                        "query", "string", "What to search for in the knowledge base", required=True
                    ),
                ],
                execute=self._run_knowledge_retrieval,
                pattern=re.compile(
                    r"^(?:knowledge|recall|remember|look up in docs?)\s+(.+)$", re.IGNORECASE
                ),
            )
        )
        self.register(
            ToolSpec(
                ...
                parameters=[
                    ToolParam(
                        "image_path", "string", "Path or URL of the image to analyze", required=True
                    ),
                ],
                execute=self._run_image_analysis,
                pattern=re.compile(
                    r"^(?:analyze|describe|what's in|look at)\s+(?:image|photo|picture)\s+(.+)$",
                    re.IGNORECASE,
                ),
            )
        )
            ...
            ToolSpec(
                ...
                parameters=[
                    ToolParam(
                        "data_path",
                        "string",
                        "Path to the data file (CSV, JSON, JSONL)",
                        required=True,
                    ),
                    ToolParam(
                        "operation",
                        "string",
                        "Analysis to perform: summary, statistics, columns, head, or a custom expression",
                        required=False,
                    ),
                ],
                execute=self._run_data_analysis,
                pattern=re.compile(
                    r"^(?:analyze|stats|summarize|describe)\s+(?:data|dataset|csv|json)\s+(.+)$",
                    re.IGNORECASE,
                ),
            )
        )
    ...
    def detect_tool_intent(self, text: str) -> tuple[str, dict[str, Any]] | None:
    ...
    async def execute(self, name: str, args: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                success=True, output=result.get("output", ""), duration_ms=elapsed, metadata=result
            )
    ...
    async def _run_web_search(self, query: str, num_results: int = 3) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "output": "Web search not available. Try installing `web_search` package or use a different tool."
            }

    async def _run_code(self, language: str, code: str) -> dict[str, Any]:
        """Execute code in a subprocess with timeout."""
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".py" if language == "python" else ".sh", delete=False
        ) as f:
            f.write(code)
            f.flush()
            fname = f.name
    ...
    async def _run_image_analysis(self, image_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                lines.append(
                    f"Embedding: {len(embedding)}-dim, L2={float((embedding**2).sum() ** 0.5):.3f}"
                )
                return {"output": "\n".join(lines)}
            except ImportError:
                return {
                    "output": f"Image: {os.path.basename(image_path)}\nFormat: {ext}\nSize: {size:,} bytes\n\n(Vision model not available)"
                }
    ...
    async def _run_data_analysis(
        self, data_path: str, operation: str = "summary"
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if ext == ".csv":
                ...
                return {
                    "output": f"CSV: {len(lines) - 1} rows, {len(headers)} columns\nHeaders: {', '.join(headers)}\nFirst row: {lines[1] if len(lines) > 1 else '(empty)'}"
                }
            elif ext in (".json",):
                data = json.loads(raw)
                ...
                return {
                    "output": f"JSON object: {len(data)} keys\nKeys: {', '.join(list(data.keys())[:20])}"
                }
            elif ext == ".jsonl":
                lines = raw.strip().split("\n")
