...
class TestChatTextDataset:
    def test_n_samples(self):
        ds = ChatTextDataset(
            "hello world", 4, {"h": 1, "e": 2, "l": 3, "o": 4, " ": 5, "w": 6, "r": 7, "d": 8}
        )
...
class TestTrainChatModel:
    ...
    def test_on_step_callback(self, tmp_path):
        ...
        calls = []
        train_chat_model(
            _pairs(4), config, on_step=lambda s, l, e, total_steps=0: calls.append((s, l, e))
        )
        assert calls
        assert all(
            isinstance(s, int) and isinstance(l, float) and isinstance(e, int) for s, l, e in calls
        )
    ...
    def test_cancel_mid_training(self, tmp_path):
        ...
        model, meta = train_chat_model(
            _pairs(4), config, on_step=stop_after_first, cancel_event=cancel
        )
...
class TestGenerateFromChatModel:
    ...
    def test_generate_greedy(self, tmp_path):
        ...
        out = generate_from_chat_model(
            model, stoi, itos, "User: Hello", max_tokens=10, temperature=0.0
        )
    ...
    def test_generate_sampling(self, tmp_path):
        model, stoi, itos = self._train_tiny(tmp_path)
        out = generate_from_chat_model(
            model, stoi, itos, "User: Hello", max_tokens=10, temperature=0.9
        )
        assert isinstance(out, str)
    ...
    def test_generate_long_prompt_truncates(self, tmp_path):
        model, stoi, itos = self._train_tiny(tmp_path)
        out = generate_from_chat_model(
            model, stoi, itos, "User: " + "x" * 100, max_tokens=5, temperature=0.0
        )
        assert isinstance(out, str)
                ...
                json.dumps(
                    {
                        "messages": [
                            {
                                "role": "user",
                                "content": "User message number one asking something interesting.",
                            },
                            {
                                "role": "assistant",
                                "content": "Assistant responds helpfully with a detailed answer about topic one.",
                            },
                        ]
                    }
                )
...
class TestCorruptedMetadata:
    ...
    def test_optimizer_state_wrong_shape_ignored(self, tmp_path):
        ...
        path = self._make_ckpt(
            tmp_path, {"optimizer_state": {"t": 5, "state": {"fake": {"m": [1, 2, 3]}}}}
        )
        config = ChatTrainConfig(
            n_embed=32,
            n_layer=1,
