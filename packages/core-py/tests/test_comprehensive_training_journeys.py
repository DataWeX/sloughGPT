            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        print(
                            f"  [RETRY] {func.__name__} attempt {attempt} failed: {exc}, retrying in {delay}s"
                        )
                        time.sleep(delay)
...
class TestTrainingLifecycle:
    ...
    def test_training_pipeline_steps_visible(self, page, collector):
        body = go(page, "/training", collector)
        has_pipeline = any(
            w in body.lower() for w in ["data", "configure", "start", "pipeline", "step"]
        )
        dt = record_devtools(
            "training_pipeline",
            collector.console_messages,
    ...
    def test_training_datasets_section(self, page, collector):
        body = go(page, "/training", collector)
        has_datasets = any(
            w in body.lower() for w in ["dataset", "data", "upload", "import", "select"]
        )
        dt = record_devtools(
            "training_datasets",
            collector.console_messages,
    ...
    def test_training_config_section(self, page, collector):
        body = go(page, "/training", collector)
        has_config = any(
            w in body.lower() for w in ["config", "epoch", "learning", "rate", "batch", "hyper"]
        )
        dt = record_devtools(
            "training_config",
            collector.console_messages,
...
class TestAutoTrainingFlow:
    ...
    def test_auto_train_has_config(self, page, collector):
        ...
        has_config = any(
            w in body.lower()
            for w in ["auto", "train", "threshold", "interval", "monitor", "enable"]
        )
        dt = record_devtools(
            "auto_train_config",
            collector.console_messages,
...
class TestChatToDPOTFlow:
    ...
    def test_chat_page_loads(self, page, collector):
        ...
        dt = record_devtools(
            "chat_load", collector.console_messages, collector.network_requests, collector.errors
        )
    ...
    def test_chat_has_input(self, page, collector):
        ...
        time.sleep(1)
        has_input = await_no_timeout(
            page, lambda: page.locator("textarea, input[type='text']").count() > 0, 5
        )
        dt = record_devtools(
            "chat_input", collector.console_messages, collector.network_requests, collector.errors
        )
    ...
    def test_training_method_dpo_available(self, page, collector):
        body = go(page, "/training", collector)
        has_dpo = (
            "dpo" in body.lower() or "preference" in body.lower() or "feedback" in body.lower()
        )
        dt = record_devtools(
            "training_dpo_available",
            collector.console_messages,
            collector.network_requests,
            collector.errors,
        )
...
class TestMonitoringFlow:
    ...
    def test_monitoring_page_loads(self, page, collector):
        ...
        has_metrics = any(
            w in body.lower() for w in ["cpu", "memory", "monitor", "health", "gpu", "system"]
        )
        dt = record_devtools(
            "monitoring_load",
            collector.console_messages,
...
class TestCrossPageNavigation:
    ...
    def test_all_routes_load_without_crash(self, page, collector, route):
        ...
        dt = record_devtools(
            f"nav_{route}", collector.console_messages, collector.network_requests, collector.errors
        )
        passed = not has_crash and len(body) > 30
        ok(
            f"nav_{route.replace('/', '_') or '_root'}",
            passed,
            f"len={len(body)}, crash={has_crash}",
            dt,
        )
...
class TestTrainingConfigInteraction:
    ...
    def test_method_selector_exists(self, page, collector):
        ...
        body = page.inner_text("body")
        has_method = any(
            w in body.lower() for w in ["method", "sft", "rlhf", "dpo", "lora", "finetune"]
        )
        dt = record_devtools(
            "config_method",
            collector.console_messages,
    ...
    def test_hyperparameter_inputs_exist(self, page, collector):
        ...
        body = page.inner_text("body")
        has_hyper = any(
            w in body.lower() for w in ["learning rate", "batch", "epoch", "dropout", "rank"]
        )
        dt = record_devtools(
            "config_hyperparams",
            collector.console_messages,
    ...
    def test_lora_toggle_exists(self, page, collector):
        ...
        dt = record_devtools(
            "config_lora", collector.console_messages, collector.network_requests, collector.errors
        )
...
class TestTrainingResults:
    ...
    def test_results_tab_loads(self, page, collector):
        ...
        dt = record_devtools(
            "results_tab", collector.console_messages, collector.network_requests, collector.errors
        )
