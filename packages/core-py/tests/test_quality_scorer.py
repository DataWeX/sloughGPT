...
class TestScorePair:
    ...
    def test_long_conversation(self):
        ...
        assistant = (
            ...
            "The process occurs in two stages: the light reactions and the Calvin cycle."
        ) * 2
    ...
    def test_incoherent_response(self):
        ...
        score_normal = score_pair(
            "What is 2+2?", "The answer to 2+2 is 4, which is a basic arithmetic operation."
        )
    ...
    def test_batch_scores_vary(self):
        pairs = [
            {"user_msg": "Hi", "assistant_msg": "Hello"},  # Short
            {
                ...
                "assistant_msg": "Python is a programming language used for many applications.",
            },  # Normal
        ]
        scores = score_batch(pairs)
        ...
        pairs = [
            {
                ...
                "assistant_msg": f"Answer {i} with enough text to be a reasonable response.",
            }
            for i in range(5)
        ]
        scores = score_batch(pairs)
...
class TestInternalBranchCoverage:
    ...
    def test_language_all_caps_string(self):
        ...
        score = _language_quality_score(
            "THIS IS ALL CAPS TEXT AND IT IS VERY LOUD AND HARD TO READ"
        )
