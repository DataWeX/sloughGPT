...
def make_gpt2_weights(vocab=8, n_embed=4, n_layers=1, seq_max=16):
    ...
    for i in range(n_layers):
        ...
        W[p + "attn.c_attn.weight"] = rng.standard_normal((n_embed * 3, n_embed)).astype(np.float32)
        ...
        W[p + "mlp.c_fc.weight"] = rng.standard_normal((ffn, n_embed)).astype(np.float32)
        W[p + "mlp.c_fc.bias"] = rng.standard_normal((ffn,)).astype(np.float32)
        W[p + "mlp.c_proj.weight"] = rng.standard_normal((n_embed, ffn)).astype(np.float32)
...
def make_llama_weights(vocab=8, n_embed=8, n_head=4, n_kv=2, n_layers=1):
    ...
    for i in range(n_layers):
        ...
        W[p + "self_attn.k_proj.weight"] = rng.standard_normal((n_embed, kv_dim)).astype(np.float32)
        W[p + "self_attn.v_proj.weight"] = rng.standard_normal((n_embed, kv_dim)).astype(np.float32)
