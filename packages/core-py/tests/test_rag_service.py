...
class TestHallucinationDetector:
    def test_detect_returns_structure(self):
        ...
        retriever = HybridRetriever(use_rerank=False)
        retriever.add_chunk(
            TextChunk(id="1", content="Paris is the capital of France", metadata={"source": "wiki"})
        )
        retriever.build_index()
    ...
    def test_unsupported_claim_detected(self):
        ...
        retriever = HybridRetriever(use_rerank=False)
        retriever.add_chunk(
            TextChunk(id="1", content="Paris is the capital of France", metadata={})
        )
        retriever.build_index()
        detector = HallucinationDetector(retriever)
