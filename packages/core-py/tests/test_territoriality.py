...
class TestTerritoryBrain:
    ...
    def test_defend_transfers_energy_and_displaces(self):
        ...
        assert np.linalg.norm(b.position - np.array([4.0, 1.0, 5.0])) == pytest.approx(
            params.defend_push
        )
    ...
    def test_push_away_shoves_one_cell_from_defender(self):
        ...
        assert np.linalg.norm(out - b.position) == pytest.approx(params.defend_push)
...
class TestTerritoryScene:
    ...
    def test_determinism_with_territoriality_on(self):
        ...
        for _ in range(2):
            ...
            sim = Simulation(scene, max_ticks=2)
            sim.run()
            outs.append((sim.summary()["defenses"], sim.summary()["defend_energy_moved"]))
...
class TestTerritoryEvolution:
    ...
    def test_benchmark_structure_and_verdict_keys(self):
        ...
        assert set(result) >= {
            ...
            "group_weight",
            "control_last_avg",
            "territoriality_last_avg",
            "defend_rate",
            "defenses",
            "defend_energy_moved",
            "territoriality_emerged",
        }
    ...
    def test_benchmark_structure_and_verdict_keys_carry_raids(self):
        ...
        assert (
            result["raid_energy_moved"]
            == result["territoriality"]["history"][-1]["raid_energy_moved"]
        )
...
class TestRaid:
    ...
    def test_raid_targets_nearest_foreign_nest(self):
        ...
        near = Nest(
            id=1,
            position=np.array([4.0, 1.0, 4.0], dtype=np.float64),
            stored_energy=50.0,
            owner_group_id=1,
        )
        far = Nest(
            id=2,
            position=np.array([6.0, 1.0, 4.0], dtype=np.float64),
            stored_energy=50.0,
            owner_group_id=2,
        )
    ...
    def test_scene_records_raid_in_summary(self):
        ...
        assert sim.summary()["raid_energy_moved"] == pytest.approx(2.0 * params.nest_draw_rate)
    ...
    def test_raider_energy_flow_is_exact(self):
        ...
        assert raider.energy == pytest.approx(
            20.0 - params.see_cost - params.passive_drain + params.nest_draw_rate,
            abs=1e-6,
        )
        assert scene.nests[0].stored_energy == pytest.approx(
            50.0 * (1.0 - params.nest_decay) - params.nest_draw_rate,
            abs=1e-6,
        )
