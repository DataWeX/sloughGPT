...
class TestSyscallHandler:
    ...
    def test_sys_wait_terminated_child(self):
        ...
        child.state = __import__('domains.shell.vm', fromlist=['ProcessState']).ProcessState.TERMINATED
        result = handler._sys_wait()
    ...
    def test_sys_serial_write(self):
        handler, cpu, _, _ = self._make_handler()
        handler._serial = __import__('domains.shell.vm', fromlist=['SerialDevice']).SerialDevice(cpu=cpu)
    ...
    def test_sys_rtc_gettime(self):
        ...
        rtc = __import__('domains.shell.vm', fromlist=['CMOSDevice']).CMOSDevice(cpu=cpu, clock=clock)
        handler._rtc = rtc
    ...
    def test_sys_disk_read(self):
        ...
        disk = __import__('domains.shell.vm', fromlist=['DiskDevice']).DiskDevice(block_device=block)
        handler._disk = disk
...
class TestPITDeviceCoverage:
    ...
    def test_read_counter_no_latch(self):
        from domains.shell.vm import X86CPU, ProcessTable, Scheduler, PITDevice
        cpu = X86CPU(memory_size=0x400000)
        ptable = ProcessTable()
        sched = Scheduler(ptable, quantum=10)
...
class TestX86VirtualSystemCoverage:
    ...
    def test_init_with_filesystem(self):
        from domains.shell.vm import X86VirtualSystem, FlatFS, BlockDevice
        block = BlockDevice()
        fs = FlatFS(block)
...
class TestDeviceRegisterMapCoverage:
    ...
    def test_register_device_no_base_raises(self):
        from domains.shell.vm import DeviceRegisterMap, DeviceFault
        drm = DeviceRegisterMap()
        mock_dev = type('Dev', (), {})()
...
class TestTrainingSyscalls:
    ...
    def test_train_start(self):
        from domains.shell.vm import X86CPU, ProcessTable, Scheduler, X86SyscallHandler, PageFrameAllocator
        cpu = X86CPU(memory_size=0x400000)
        ptable = ProcessTable()
        sched = Scheduler(ptable, quantum=10)
        alloc = PageFrameAllocator(total_memory=0x400000, reserved_ranges=[(0, 0x100000)])
...
class TestCPUTensorOps:
    ...
    def test_fdiv_zero(self):
        ...
        cpu.regs[0] = 10.0
        cpu.regs[1] = 0.0
        from domains.shell.vm import _op_fdiv, InsFault
        with pytest.raises(InsFault):
            _op_fdiv(cpu, ['R2', 'R0', 'R1'])
...
class TestRemainingSmallBlocks:
    ...
    def test_resolve_label_digit(self):
        from domains.shell.vm import _resolve_label, CPU
        cpu = CPU()
        assert _resolve_label(cpu, "42") == 42

    def test_resolve_label_invalid(self):
        from domains.shell.vm import _resolve_label, CPU, InsFault
        cpu = CPU()
        with pytest.raises(InsFault):
