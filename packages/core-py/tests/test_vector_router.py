...
class TestGetStats:
    ...
    def test_with_store(self, client):
        ...
        with patch(
            "apps.api.server.routers.vector.VectorRouter.get_vector_store",
...
class TestInitVectorStore:
    def test_init_in_memory(self, client):
        fake_store = FakeVectorStore()
        with patch(
            "domain.inference._internal.vector_store.create_vector_store",
            new_callable=AsyncMock,
            return_value=fake_store,
        ):
    ...
    def test_init_fallback_on_import_error(self, client):
        ...
        with patch(
            "domain.inference._internal.vector_store.create_vector_store", side_effect=fake_create
        ):
