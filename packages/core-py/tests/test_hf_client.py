...
class TestHFClient:
    def test_api_mode_init(self):
        with (
            patch("domain.training._internal.huggingface.client._APILoader") as MockLoader,
            patch("domain.training._internal.huggingface.client._LocalLoader"),
        ):
    ...
    def test_local_mode_init(self):
        with (
            patch("domain.training._internal.huggingface.client._APILoader"),
            patch("domain.training._internal.huggingface.client._LocalLoader") as MockLoader,
        ):
    ...
    def test_api_mode_kwarg_passthrough(self):
        with (
            ...
            HFClient("gpt2", mode="api", temperature=0.7)
            MockLoader.assert_called_once()
...
class TestGetModelMemory:
    def test_returns_dict(self):
        with patch(
            "domain.training._internal.huggingface.client.get_model_requirements"
        ) as mock_req:
    ...
    def test_passes_precision(self):
        with patch(
            ...
            get_model_memory("gpt2", precision="int8")
            mock_req.assert_called_once_with("gpt2")
...
class TestListModels:
    def test_list_all(self):
        ...
        with (
            patch("domain.training._internal.huggingface.model_map.search_models") as mock_search,
            patch("domain.training._internal.huggingface.model_map.ModelSize"),
        ):
    ...
    def test_list_with_both_filters(self):
        with (
            ...
            mock_search.return_value = []
            list_models(organization="openai", size="medium")
            mock_search.assert_called_once()
...
class TestConvenienceFunctions:
    ...
    def test_generate_default_model(self):
        with patch("domain.training._internal.huggingface.client.HFClient") as MockClient:
            ...
            generate("prompt")
            MockClient.assert_called_once_with("meta-llama/Llama-2-7b-chat-hf", mode="api")
