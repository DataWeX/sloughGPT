...
from domain.learner._internal.knowledge import KnowledgeFact, KnowledgeMemory
...
class TestAddFactsBatch:
    ...
    def test_add_facts_dedup_against_existing(self, km):
        ...
        assert (
            km.add_facts(
                [KnowledgeFact(content="existing fact content here", topic="t", source="test")]
            )
            == 0
        )
    ...
    def test_delete_by_id(self, km):
        km.add_fact(
            KnowledgeFact(content="Delete me", topic="test", source="test", timestamp=time.time())
        )
        items = km.list_all()
        assert len(items) == 1
        item_id = items[0]["id"]
    ...
    def test_delete_frees_content_hash(self, km):
        ...
        km.add_fact(
            KnowledgeFact(
                content="Re-addable content", topic="test", source="test", timestamp=time.time()
            )
        )
        item_id = km.list_all()[0]["id"]
        km.delete_by_id(item_id)
        re_added = km.add_fact(
            KnowledgeFact(
                content="Re-addable content", topic="test", source="test", timestamp=time.time()
            )
        )
    ...
    def test_update_fact_keeps_topic_when_omitted(self, km):
        km.add_fact(
            KnowledgeFact(
                content="Keep topic fact", topic="retained", source="test", timestamp=time.time()
            )
        )
        item_id = km.list_all()[0]["id"]
    ...
    def test_update_fact_unknown_id_returns_false(self, km):
        km.add_fact(
            KnowledgeFact(content="Some fact", topic="test", source="test", timestamp=time.time())
        )
    ...
    def test_update_fact_to_duplicate_returns_false(self, km):
        km.add_fact(
            KnowledgeFact(content="First fact", topic="test", source="test", timestamp=time.time())
        )
        km.add_fact(
            KnowledgeFact(content="Second fact", topic="test", source="test", timestamp=time.time())
        )
    ...
    def test_updated_fact_can_be_restored_to_original_content(self, km):
        km.add_fact(
            KnowledgeFact(content="Original", topic="test", source="test", timestamp=time.time())
        )
        item_id = km.list_all()[0]["id"]
    ...
    def test_clear_all_empties_store(self, km):
        km.add_fact(
            KnowledgeFact(
                content="Item to clear", topic="test", source="test", timestamp=time.time()
            )
        )
    ...
    def test_clear_all_also_clears_visited(self, km):
        km.add_fact(
            KnowledgeFact(
                content="Clear visited test", topic="test", source="test", timestamp=time.time()
            )
        )
        km.clear_all()
        re_added = km.add_fact(
    ...
    def test_clear_all_persists_empty_file(self, km):
        ...
        km.add_fact(
            KnowledgeFact(
                content="Persist clear test", topic="test", source="test", timestamp=time.time()
            )
        )
    ...
    def test_clear_all_survives_restart(self, km):
        km.add_fact(
            KnowledgeFact(
                content="Restart clear test", topic="test", source="test", timestamp=time.time()
            )
        )
        assert km.clear_all() == 1
    ...
    def test_stats_returns_dict(self, km):
        km.add_fact(
            KnowledgeFact(content="Stats fact", topic="test", source="test", timestamp=time.time())
        )
        stats = km.stats()
    ...
    def test_many_facts(self, km):
        for i in range(50):
            km.add_fact(
                KnowledgeFact(
                    content=f"Fact number {i}", topic="bulk", source="test", timestamp=time.time()
                )
            )
...
class TestAutoIngestFromChat:
    ...
    def test_extract_facts_from_text_with_facts(self):
        ...
        text = (
            "Python is a programming language. It has dynamic typing. The current version is 3.12."
        )
