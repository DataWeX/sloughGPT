"""
Tests for the live realm view (``domain.shell.realm_view.py``).
...
class TestMakeLiveScene:
    ...
    def test_deterministic_scene(self):
        ...
        for ba, bb in zip(a.babies, b.babies, strict=False):
    ...
    def test_seasons_year_length(self):
        scene = make_live_scene(
            grid=(16, 8, 16), population=4, day_ticks=8, seasons_per_year=4, seed=7
        )
        assert scene.params.solar_season_ticks == 32  # 8 ticks/day * 4 days
        assert scene.params.solar_seasonality == 1.0

    def test_seasons_deterministic_scene(self):
        a = make_live_scene(grid=(16, 8, 16), population=6, day_ticks=8, seasons_per_year=4, seed=5)
        b = make_live_scene(grid=(16, 8, 16), population=6, day_ticks=8, seasons_per_year=4, seed=5)
        assert np.allclose(a.world.energy, b.world.energy)
...
class TestSeasons:
    ...
    def _stepped(self, ticks: int, seasons_per_year: int = 4) -> SimScene:
        scene = make_live_scene(
            grid=(16, 8, 16), population=6, day_ticks=8, seasons_per_year=seasons_per_year, seed=7
        )
    ...
    def test_year_envelope_bar_rendered_only_with_seasons(self):
        ...
        off = self._frame(
            make_live_scene(grid=(16, 8, 16), population=6, day_ticks=8, seed=7), tick=4
        )
