...
class TestPointGenerate:
    def test_periodic(self):
        p = Point(identity="t", function_type="periodic", params={"a": 1.0, "b": 0.5, "w": 0.1})
        vals = p.generate(10)
    ...
    def test_linear(self):
        p = Point(identity="l", function_type="linear", params={"a": 2.0, "b": 1.0})
        vals = p.generate(5)
        i = np.arange(5, dtype=np.float32)
        np.testing.assert_allclose(vals, 2.0 * i + 1.0, atol=1e-6)

    def test_polynomial(self):
        p = Point(identity="p", function_type="polynomial", params={"a": 0.5, "b": 1.0, "c": 2.0})
        vals = p.generate(5)
        i = np.arange(5, dtype=np.float32)
...
class TestPointNbytes:
    ...
    def test_linear_no_residual(self):
        p = Point(identity="l", function_type="linear", params={"a": 1.0, "b": 0.0})
        assert p.nbytes() > 0
    ...
    def test_raw(self):
        ...
        data = b"hello world"
        p = Point(
            identity="r", function_type="raw", params={"data_b64": base64.b64encode(data).decode()}
        )
...
class TestPointBytesRoundTrip:
    ...
    def test_linear(self):
        p = Point(identity="l", function_type="linear", params={"a": 2.0, "b": 3.0})
        data = p.to_bytes()
        p2 = Point.from_bytes(data)
...
class TestPointDictRoundTrip:
    ...
    def test_with_residual(self):
        ...
        p = Point(identity="l", function_type="linear", params={"a": 1.0, "b": 0.0}, residual=res)
        d = p.to_dict()
    ...
    def test_accuracy_preserved(self):
        p = Point(identity="t", function_type="linear", params={"a": 1.0, "b": 0.0}, accuracy=0.95)
        d = p.to_dict()
        p2 = Point.from_dict(d)
    ...
    def test_raw(self):
        ...
        p = Point(
            identity="r",
            function_type="raw",
            params={"data_b64": base64.b64encode(b"x").decode(), "dtype": "float32", "shape": []},
        )
        d = p.to_dict()
        p2 = Point.from_dict(d)
        assert p2.function_type == "raw"
