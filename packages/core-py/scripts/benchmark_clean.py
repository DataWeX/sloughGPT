...
from domain.training._internal.slonet import SloLSTM, Tensor, no_grad
...
def bench(name, fn_sn, fn_th=None):
    ...
    if fn_th:
        ...
        label += (
            f"  Torch: {th_ms:>8.2f} ms  {'🏆 FASTER' if ratio < 1 else f'{ratio:.2f}x slower'}"
        )
    print(label)
    return sn_ms
...
for M, N, K in [(256, 256, 256), (512, 512, 512), (1024, 1024, 1024)]:
    ...
    bench(f"matmul ({M}x{K}) @ ({K}x{N})", make_sn(A, B), make_th(A, B) if HAS_TORCH else None)
...
for vocab, seq, hidden in [(256, 16, 256), (1000, 32, 512)]:
    ...
    if HAS_TORCH:
        ...
        bench(f"LSTM 2L-{hidden} ({vocab}v/{seq}s)", make_sn(sn_lstm, x_np), make_th(th_lstm, tx))
    else:
        bench(f"LSTM 2L-{hidden} ({vocab}v/{seq}s)", make_sn(sn_lstm, x_np))
...
for B, H, N, E in [(4, 4, 32, 16), (8, 4, 128, 64)]:
    ...
    if HAS_TORCH:
        ...
        bench(f"attention {B}x{H}x{N}x{E}", make_sn(q, k, v), make_th(tq, tk, tv))
    else:
        bench(f"attention {B}x{H}x{N}x{E}", make_sn(q, k, v))
