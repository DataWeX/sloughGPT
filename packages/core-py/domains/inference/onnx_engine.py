...
class ONNXInferenceEngine:
    ...
    def _load_model(self):
        ...
        logger.info(f"Loading {self.model_name} for ONNX Runtime...", extra={"tag": "INF"})
        t0 = time.time()
        ...
        if not os.path.exists(onnx_path):
            logger.info("Exporting to ONNX format (first time, takes a minute)...", extra={"tag": "INF"})
        ...
        logger.info(f"Model loaded in {time.time() - t0:.1f}s", extra={"tag": "INF"})
        logger.info(f"Using: %s (%s)", self.execution_provider, self.device, extra={"tag": "INF"})
    ...
    def _export_to_onnx(self, output_path: str):
        ...
        logger.info("Exported ONNX model to %s", output_path, extra={"tag": "INF"})
