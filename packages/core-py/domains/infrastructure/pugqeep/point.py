...
class Point(PointProtocol):
    ...
    residual: np.ndarray | None = None  # difference from exact
    ...
    def generate(self, n: int) -> np.ndarray:
        ...
        if self.function_type == "cluster":
            ...
            if huffman_data is not None:
                ...
                assignments = HuffmanTree.decode(huffman_data, total_bits, tree, len(assignments))
        ...
        if function_type == "periodic":
            ...
            if len(param_bytes) > offset:
                ...
                if has_res:
                    ...
                    residual = np.frombuffer(
                        param_bytes[offset : offset + res_len], dtype=np.float32
                    )
            ...
            if is_quantized:
                centroids = np.frombuffer(
                    param_bytes[offset : offset + n_centroids], dtype=np.uint8
                )
                offset += n_centroids
                ...
                centroids = np.frombuffer(
                    param_bytes[offset : offset + n_centroids * 4], dtype=np.float32
                )
            ...
            assignments = np.frombuffer(
                param_bytes[offset : offset + n_assignments], dtype=np.uint8
            )
