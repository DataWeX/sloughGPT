...
ONNX export builds a torch-compatible mirror of the SloNet model and
delegates to ``torch.onnx.export``. The module imports cleanly without
torch installed; the export path itself requires PyTorch and raises a
clear ``ImportError`` when it is unavailable.
"""
...
if _TORCH_AVAILABLE:  # pragma: no cover (torch-only classes)
    ...
    class SloughGPTONNXExport:
        """Torch-free placeholder — real export requires PyTorch."""
...
def export_sloughgpt_to_onnx(model, output_path: str, example_input=None, config: Optional[ONNXExportConfig] = None, seq_len: int = 128) -> str:
