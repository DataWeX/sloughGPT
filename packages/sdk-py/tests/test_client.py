...
from sloughgpt_sdk.client import ChatMessage, SloughGPTClient
...
class TestExportTrainingHistory:
    def test_json_export(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"format": "json", "outcomes": [], "count": 0}
        )
        result = client.export_training_history("json", 10)
        client._mock_request.assert_called_once_with(
            "GET", "/settings/training/history/export?format=json&limit=10"
        )
        assert result["format"] == "json"

    def test_csv_export(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"format": "csv", "content": "a,b\n1,2", "count": 1}
        )
        result = client.export_training_history("csv")
        client._mock_request.assert_called_once_with(
            "GET", "/settings/training/history/export?format=csv&limit=0"
        )
...
class TestGenerateModelCard:
    def test_generate(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"card": {"model_name": "test"}, "markdown": "# test"}
        )
...
class TestGetDashboardSummary:
    def test_get_summary(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"health": {"model_loaded": True}}
        )
...
class TestCompareTrainingRuns:
    def test_compare(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"run_a": {}, "run_b": {}, "differences": {}}
        )
        result = client.compare_training_runs("run-1", "run-2")
        client._mock_request.assert_called_once_with(
            "GET", "/settings/training/compare?run_a=run-1&run_b=run-2"
        )
...
class TestGetBatchTrainingStatus:
    def test_get_status(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"jobs": [], "summary": {"total": 0}}
        )
...
class TestListTrainingPresets:
    def test_list_presets(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"presets": [{"name": "quick-finetune"}]}
        )
...
class TestGetTrainingPreset:
    def test_get_preset(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"name": "Quick Fine-Tune", "model": "gpt2"}
        )
        result = client.get_training_preset("quick-finetune")
        client._mock_request.assert_called_once_with(
            "GET", "/settings/training/presets/quick-finetune"
        )
...
class TestApplyTrainingPreset:
    def test_apply_preset(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"preset": "quick-finetune", "applied": {}}
        )
        result = client.apply_training_preset("quick-finetune")
        client._mock_request.assert_called_once_with(
            "POST", "/settings/training/presets/quick-finetune/apply"
        )
...
class TestGetTrainingRun:
    def test_get_run(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"run_id": "run-1", "model": "gpt2"}
        )
...
class TestDeleteTrainingRun:
    def test_delete_run(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"deleted": True, "run_id": "run-1"}
        )
...
class TestFilterTrainingRuns:
    def test_filter_runs(self, client):
        ...
        client.filter_training_runs(model="gpt2", limit=10)
        client._mock_request.assert_called_once()
...
class TestClearTrainingHistory:
    def test_clear_history(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"cleared": True, "removed_count": 5}
        )
...
class TestAddRunTag:
    def test_add_tag(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"run_id": "run-1", "tags": ["best"]}
        )
...
class TestSetRunNotes:
    def test_set_notes(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"run_id": "run-1", "notes": "test note"}
        )
...
class TestGetRunsByTag:
    def test_get_runs_by_tag(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"runs": [], "count": 0, "tag": "best"}
        )
...
class TestExportTrainingRun:
    def test_export_run(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"run_id": "run-1", "format": "json", "content": "{}"}
        )
        result = client.export_training_run("run-1", "json")
        client._mock_request.assert_called_once_with(
            "GET", "/settings/training/runs/run-1/export?format=json"
        )
        assert result["run_id"] == "run-1"
...
class TestToggleBookmark:
    def test_toggle_bookmark(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"run_id": "run-1", "bookmarked": True}
        )
        result = client.toggle_bookmark("run-1")
        client._mock_request.assert_called_once_with(
            "POST", "/settings/training/runs/run-1/bookmark"
        )
        assert result["run_id"] == "run-1"
...
class TestDuplicateTrainingRun:
    def test_duplicate_run(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"run_id": "new-run", "model": "gpt2"}
        )
...
class TestBulkDeleteRuns:
    def test_bulk_delete(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"deleted_count": 2, "requested": 2}
        )
...
class TestBulkAddTag:
    def test_bulk_add_tag(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"updated_count": 2, "tag": "best"}
        )
...
class TestBulkBookmark:
    def test_bulk_bookmark(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"updated_count": 2, "bookmarked": True}
        )
...
class TestAutoTrainStatus:
    def test_get_auto_train_status(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"enabled": True, "threshold": 10}
        )
...
class TestAutoTrainConfig:
    def test_update_auto_train_config(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"enabled": True, "threshold": 20}
        )
    ...
    def test_get_training_analytics(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {
                ...
                "quality_trend": [],
                "method_distribution": {"finetune": 3, "lora": 2},
                "model_distribution": {"gpt2": 5},
                "best_run": None,
                "recent_runs": [],
                "tag_cloud": {},
            }
...
class TestSecurityKeys:
    def test_list_keys(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"keys": [{"id": "k1"}], "count": 1}
        )
    ...
    def test_rotate_key(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"id": "k1", "new_secret": "s3cret"}
        )
...
class TestTenants:
    def test_list_tenants(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"tenants": [{"id": "t1"}], "count": 1}
        )
...
class TestProfiles:
    def test_list_profiles(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"profiles": [{"id": "p1"}], "count": 1}
        )
    ...
    def test_recommend_profile(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"profile_id": "p2", "reason": "best match"}
        )
...
class TestWorkspaces:
    def test_list_workspaces(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"workspaces": [{"id": "w1"}], "count": 1}
        )
...
class TestGetToken:
    def test_exchange_api_key(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"access_token": "jwt-123", "token_type": "bearer"}
        )
        result = client.get_token("my-api-key")
        client._mock_request.assert_called_once_with(
            "POST", "/auth/token", json={"api_key": "my-api-key"}
        )
...
class TestHealth:
    def test_health(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"status": "healthy", "model_loaded": True, "model_type": "gpt2"}
        )
...
class TestSettings:
    def test_get_settings(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"generation": {"temperature": 0.7}}
        )
    ...
    def test_get_generation_settings(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"temperature": 0.8, "top_p": 0.9}
        )
    ...
    def test_get_adaptive_insights(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"exploration_rate": 0.3, "confidence": 0.8}
        )
...
class TestModels:
    def test_list_models(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"models": [{"model_id": "gpt2", "name": "GPT-2"}], "count": 1}
        )
    ...
    def test_load_model(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"loaded": True, "model_id": "gpt2"}
        )
    ...
    def test_get_current_model(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"model_id": "gpt2", "loaded": True}
        )
...
class TestSessions:
    def test_create_session(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"session_id": "s1", "created_at": "2026-09-10"}
        )
    ...
    def test_list_sessions(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"sessions": [{"session_id": "s1"}]}
        )
    ...
    def test_get_session(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"session_id": "s1", "messages": []}
        )
    ...
    def test_get_session_messages(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"messages": [{"role": "user", "content": "hi"}]}
        )
...
class TestKnowledge:
    def test_list_knowledge(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"items": [{"id": "k1", "content": "test"}]}
        )
    ...
    def test_add_knowledge(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"id": "k2", "content": "new fact"}
        )
    ...
    def test_search_knowledge(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"results": [{"id": "k1", "score": 0.95}]}
        )
...
class TestDatasets:
    def test_list_datasets(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"datasets": [{"dataset_id": "d1", "name": "train"}], "count": 1}
        )
    ...
    def test_get_dataset(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"id": "d1", "name": "train", "rows": 100}
        )
    ...
    def test_get_dataset_stats(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"rows": 100, "columns": 5, "size_bytes": 1024}
        )
...
class TestExperiments:
    ...
    def test_list_experiments(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"experiments": [{"id": "exp1"}]}
        )
...
class TestRateLimit:
    def test_get_rate_limit_status(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"requests_remaining": 100, "reset_at": "2026-09-10T12:00:00Z"}
        )
...
class TestAuditLog:
    def test_get_audit_log(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"logs": [{"action": "login", "timestamp": "2026-09-10"}]}
        )
...
class TestRegistry:
    ...
    def test_get_registry_best(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"model_id": "best-m", "score": 0.99}
        )
    ...
    def test_get_registry_stats(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"total_models": 5, "avg_score": 0.85}
        )
...
class TestBenchmark:
    def test_run_benchmark(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"job_id": "b1", "status": "running"}
        )
    ...
    def test_get_benchmark_metrics(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"metrics": [{"name": "latency", "value": 50}]}
        )
    ...
    def test_get_benchmark_stats(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"total_runs": 10, "avg_latency": 45}
        )
...
class TestTokenizer:
    def test_get_tokenizer_stats(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"vocab_size": 50257, "token_count": 1000}
        )
...
class TestMetrics:
    def test_metrics(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"requests_total": 100, "requests_success": 95}
        )
...
class TestGenerate:
    def test_generate(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"generated_text": "Hello world", "model": "gpt2", "inference_time_ms": 50}
        )
    ...
    def test_quick_generate(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"generated_text": "Quick result", "model": "gpt2"}
        )
...
class TestChat:
    ...
    def test_quick_chat(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {
                "choices": [{"message": {"role": "assistant", "content": "Quick reply"}}],
                "model": "gpt2",
            }
        )
...
class TestSouls:
    def test_list_souls(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"souls": [{"name": "default"}], "count": 1}
        )
    ...
    def test_get_current_soul(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"name": "default", "active": True}
        )
    ...
    def test_switch_soul(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"switched": True, "name": "creative"}
        )
...
class TestKnowledgeExtended:
    def test_get_knowledge_stats(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"total_items": 100, "topics": 5}
        )
    ...
    def test_ingest_knowledge_url(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"id": "k3", "url": "https://example.com"}
        )
...
class TestTokenizerExtended:
    def test_tokenize(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"tokens": [1, 2, 3], "count": 3}
        )
    ...
    def test_train_tokenizer(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"vocab_size": 1000, "trained": True}
        )
...
class TestSystem:
    def test_get_system_metrics(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"cpu_percent": 50, "memory_percent": 60}
        )
    ...
    def test_get_system_info(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"platform": "linux", "python": "3.12"}
        )
    ...
    def test_get_system_disk(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"total_gb": 500, "used_gb": 200}
        )
...
class TestCompanion:
    def test_get_personalities(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"personalities": [{"name": "friendly"}]}
        )
    ...
    def test_set_personality(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"personality": "friendly", "set": True}
        )
    ...
    def test_list_companion_presets(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"presets": [{"name": "default"}]}
        )
...
class TestDatasetsExtended:
    def test_import_dataset_local(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"dataset_id": "d2", "name": "local"}
        )
    ...
    def test_import_dataset_github(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"dataset_id": "d3", "name": "github"}
        )
    ...
    def test_import_dataset_url(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"dataset_id": "d4", "name": "url"}
        )
...
class TestTrainingCore:
    def test_start_training(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"job_id": "t1", "status": "pending"}
        )
    ...
    def test_get_training_status(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"job_id": "t1", "status": "running", "progress": 0.5}
        )
    ...
    def test_list_training_jobs(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"jobs": [{"job_id": "t1"}], "count": 1}
        )
    ...
    def test_get_training_recovery_stats(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"recoverable": 2, "last_checkpoint": "ckpt-5"}
        )
...
class TestAutoTrainExtended:
    ...
    def test_list_auto_train_checkpoints(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"checkpoints": [{"name": "ckpt-1"}]}
        )
...
class TestVQA:
    def test_ask_question(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"answer": "a cat", "confidence": 0.9}
        )
        mock_file = MagicMock()
    ...
    def test_detect_objects(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"objects": [{"label": "cat", "bbox": [0, 0, 100, 100]}]}
        )
        mock_file = MagicMock()
        mock_file.__enter__ = MagicMock(return_value=mock_file)
        mock_file.__exit__ = MagicMock(return_value=False)
    ...
    def test_analyze_pdf(self, client):
        client._mock_request.return_value = MagicMock(
            json=lambda: {"analysis": "This is a report", "pages": 5}
        )
        mock_file = MagicMock()
        mock_file.__enter__ = MagicMock(return_value=mock_file)
        mock_file.__exit__ = MagicMock(return_value=False)
...
class TestDocstore:
    ...
    def test_put_docstore_doc(self, client):
        client._mock_request.return_value = MagicMock(json=lambda: {"id": "d1", "status": "stored"})
    ...
    def test_delete_docstore_doc(self, client):
        client._mock_request.return_value = MagicMock(json=lambda: {"deleted": "d1"})
    ...
    def test_clear_docstore_collection(self, client):
        client._mock_request.return_value = MagicMock(json=lambda: {"cleared": 5})
    ...
    def test_bulk_put_docstore(self, client):
        client._mock_request.return_value = MagicMock(json=lambda: {"stored": 3})
...
class TestCollections:
    ...
    def test_delete_collection(self, client):
        client._mock_request.return_value = MagicMock(json=lambda: {"deleted": "c1"})
    ...
    def test_collect_from_collection(self, client):
        client._mock_request.return_value = MagicMock(json=lambda: {"collected": 50})
