...
.effort{font-size:11px;color:var(--primary);font-weight:600;margin-left:6px}
.sprint{font-size:11px;color:var(--accent);font-weight:600;margin-left:6px}
...
function cardHtml(c) {
  ...
  const effort = c.effort ? `<span class="effort">${c.effort}pts</span>` : "";
  const sprint = c.sprint ? `<span class="sprint">${esc(c.sprint)}</span>` : "";
  ...
  return `<div class="card" draggable="true" data-id="${esc(c.id)}" title="${esc(c.description||"")}">
    ...
    <div class="meta">${PICO[c.priority] ? `<span class="pri">${PICO[c.priority]}</span>` : ""}${effort}${sprint}${tags}${n}${due}${overdue}${blocked}</div>
  </div>`;
}
