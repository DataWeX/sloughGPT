...
**Stage 10 — IN-WORLD LIFE CYCLE (birth / death).**
An opt-in reproduction channel that closes the loop Stage 5 left open: until
now the evolution engine re-seeded each generation, so a scene could never
grow its own population inside the tick loop. `lifecycle_enabled` lets a
parent grant a birth directly inside the simulation. A baby that clears the
energy threshold (`reproduce_energy_threshold`) and has room under
`max_entities` reads its body state (energy, grid position) and its
`perceptron_reproduce` gate (entity-input → 1 sigmoid, zero-initialized so the
four behavior brains draw identically with the channel off, weights drawn last
from a dedicated RNG stream); when the gate clears `reproduce_gate_threshold`
the parent opens a birth. The cost is a transfer, never creation: the nearest
nest of the parent's tribe funds `birth_cost * birth_nest_fraction` from its
bank, the parent pays the remainder, and the child spawns on torus-wrapped
ground within `birth_range` of the parent — `parent.energy + nest_bank +
child.energy` is conserved exactly, and an unfundable birth aborts with a
refund. The child copies the parent's perceptron channels and best memories
(and joins its tribe), so a scene now sustains itself across ticks: babies
starve out, births replace them, and `Simulation` reports `reproduced`,
`birth_energy`, `child_id` per tick and `births`/`birth_energy_moved`/
`deaths`/`alive_count` in the summary. CLI: `--lifecycle`.

**Stage 11 — DIVISION OF LABOR (Builder / Warrior postures).**
An opt-in specialization channel that gives each baby a heritable ROLE.
`specialization_enabled` adds a `perceptron_role` (body-input → 1 gate) whose
value is a posture: below `role_gate_threshold` (0.5) the baby is a BUILDER,
at or above it a WARRIOR. A Builder that carries genuine surplus (energy above
`start_energy`) and stands within `nest_use_radius` of its OWN tribe's nearest
nest deliberately banks `role_deposit_fraction` (0.1) of that surplus into the
bank — a deliberate transfer that replaces the noisy cell-write deposits and
lifts the tribe's famine floor. A Warrior standing within `territory_radius`
of a FOREIGN tribe's nearest nest raids that bank even when it is not hungry,
capped at `role_raid_fraction` (0.5) of `nest_draw_rate` and the bank itself —
a warrior lifts the tribe's mean. Both acts are pure transfers (the
conservation invariant holds — a builder never banks below its start-energy
buffer, a warrior never drains more than the bank holds), and both land in the
same tick's honest net reward (step 8b), so each posture is selected for only
where it pays. The role brain is built from fixed zeros when the channel is
off, and its genome weights draw from a dedicated RNG stream, so the four
behavior brains stay bit-identical with the channel off — the locked selection
proofs keep their exact genome layout and energy flow. Trait-group selection
compares tribes by the geometric mean of member energy, so complementary
...
**Stage 14 — THE SEASONAL YEAR ENVELOPE (the diurnal cycle rides a year).**
Stage 13's sun follows the same fixed half-wave curve every day. Stage 14
wraps that curve inside a slower cosine year — `solar_season_ticks` ticks
long — so a summer noon outshines a winter noon while the sky still deposits
ONLY onto the exposed surface cells of the boundary. `solar_seasonality`
scales the swing (0 = flat diurnal mean, 1 = full swing), and
`solar_season_ticks` is set to `seasons_per_year * solar_day_ticks` by the
benchmark. The year adds no new physics: the sun is still the only generator,
entering only at the boundary, so the conservation tripwire absorbs it
exactly as in Stage 13. `benchmark_seasons` runs two arms on the SAME
generated world (plain Stage 13 diurnal cycle vs. the year envelope, same
seed, in-life learning on, every other channel off) and verifies the four
invariants: conservation exact under the seasonal boundary
(`seasonal_conservation_exact`), the closed-world tripwire still strictly
monotonic with the sky off (`closed_monotonic`), RNG isolation — the year
envelope consumes no RNG, so same-seed genome draws are bit-identical on/off
(`brains_identical`) — and season liveness: a summer noon is brighter than a
winter noon, the seasonal arm sees sunshine and a positive boundary deposit,
and conservation survived the boundary (`seasons_emerged`). Defaults off, so
the locked selection proofs keep their exact genome layout and energy flow.
Demonstrated end-to-end (`seasonal_conservation_exact=yes`,
`closed_monotonic=yes`, `brains_identical=yes`,
`summer_noon=0.96 > winter_noon=0.04`, `seasons_emerged=yes`) via
`python3 -m domains.shell.world_driver --seasons`. CLI: `--seasons`.
The live realm view shows the same physics: `realm_live --seasons` names the
current season in the skyline (`year N SUMMER/AUTUMN/WINTER/SPRING day D/D
...`), tints the sun by season (hot yellow summer, pale cyan winter), and
draws a second bar tracing the daylight envelope across the whole year with
the current tick highlighted — a bright summer ridge reading into a dark
midwinter trough. CLI: `realm_live --seasons [--seasons-per-year N]
[--seasonality F]`.
...
| In-World Life Cycle (Birth + Death) | `simulation.py`, `evolution.py`, `world_driver.py` | Opt-in (`lifecycle_enabled`) reproduction channel that lets a scene grow its own population inside the tick loop instead of being re-seeded each generation. Each baby gains a `perceptron_reproduce` (body-input → 1 sigmoid gate, zero-initialized — no RNG draw — and the weights draw last from a dedicated RNG stream, so the four behavior brains stay bit-identical with the channel off). A baby with energy above `reproduce_energy_threshold` (and room under `max_entities`) reads its body state and, when the gate clears `reproduce_gate_threshold`, opens a birth via `SimScene.birth(parent)`: the nearest nest of the parent's tribe funds `birth_cost * birth_nest_fraction` from its bank, the parent pays the remainder (an unfundable birth aborts with a refund), and `spawn_child()` copies the parent's perceptron channels + best memories and places the child on torus-wrapped ground within `birth_range`. Birth is a transfer, never creation — `parent.energy + nest_bank + child.energy` conserves exactly, preserving the world's conservation invariant. Death already shared the tick loop (starvation sweeps dead babies and deposits their episodes into the world reservoir); with births added, births and deaths are one in-tick life cycle. Tick rows carry `reproduced`/`birth_energy`/`child_id`; the summary reports `births`/`birth_energy_moved`/`deaths`/`alive_count`. Genome and scene serialization carry the reproduce brain only when the channel is on. CLI: `--lifecycle` |
| Lifecycle Emergence Proof | `evolution.py`, `world_driver.py` | `benchmark_lifecycle()` runs two arms on the SAME grouped world (same terrain, same per-group food pools, same per-tribe spawn territories, same initial core genomes — the reproduce brain draws from a dedicated stream so the behavior brains are bit-identical): `lifecycle_enabled` off vs. on. Reproduction is never hardcoded — the gate starts at zero (sigmoid(0) = 0.5, exactly on the threshold) and selection must push it open or keep it closed by what births actually cost. Verdict: lifecycle-arm final average fitness vs. the control arm's, plus `births`, `birth_energy_moved`, `deaths`, `alive_count`, and the `lifecycle_emerged` flag (the lifecycle arm ever granted a birth). CLI: `--lifecycle` |
| 30 Tests | `test_lifecycle.py` | Reproduce brain (zero-init gate sits on the threshold, open gate clears it, off-channel zero-init keeps the four behavior brains' RNG layout untouched), birth mechanics (gate-gated grant, conservation: child's `birth_cost` equals parent + nest losses exactly, nest funds `birth_nest_fraction` and the parent the rest, abort + refund when the parent cannot fund, `max_entities` cap), scene (child placement/range/tribe/inheritance, births counter, off-by-default runs no births), simulation (tick-loop births, energy-threshold gating, starvation deaths remove babies every tick, result-row fields, summary counts, serialization round-trip), evolution (genome round-trip + `from_baby`, dedicated-stream lock — four behavior brains bit-identical off vs. on, run history carries lifecycle fields, benchmark keys + control arm never breeds + determinism) — all pass |
...
| 22 Tests | `test_realm_view.py` | Live-view construction (solar-lit generated world with `cells_input_dim` 6, all babies spawned alive on the surface, seeded determinism), frame rendering (sun glyph present at noon and absent before the first tick, material glyphs + legend, every alive baby overlaid as a `B`, grayscale heat backgrounds written), the live loop (`live_view` returns the full stats dict, emits one cleared frame per tick, seeded runs bit-identical, birth/death counters tracked in an engine-driven scene), and the Stage 14 seasonal year (scene year length from `seasons_per_year * day`, deterministic envelope, year advance + season names cycle, summer noon outshines winter noon, skyline names the current season, year envelope bar only when seasons on, summer peak ridge at the bar's high end) — all pass |
| World Driver | `world_driver.py` | Headless observability harness: `WorldDriver` builds a world (empty or deterministic terrain), runs N ticks via the real simulation loop, and reports per-tick energy economy, material populations, and a conservation invariant (grid+entity energy must never increase — a physics regression tripwire). `run_evolution()` delegates to `EvolutionEngine` for genetic sweeps; `--emergence` runs the emergence benchmark; `--social` runs the trait-group-vs-individual cooperation benchmark; `--messages` switches on the directed inter-agent message channel; `--structures` switches on durable nest building; `--culture` runs the teaching-vs-vertical-only cultural transmission benchmark; `--memory` runs the world-reservoir-vs-memotype-only long-term memory benchmark; `--predation` runs the predator-prey-on-vs-off benchmark (Stage 8); `--territory` runs the territoriality-on-vs-off defense+raid benchmark (Stage 9, reports `defenses`, `defend_rate`, `defend_energy_moved`, `raids`, `raid_energy_moved`); `--lifecycle` runs the in-world birth/death-on-vs-off benchmark (Stage 10, reports `births`, `birth_energy_moved`, `deaths`, `alive_count`, `lifecycle_emerged`); `--civilization` runs the integrated Stage 12 proof (all channels on at once, reports `conservation_monotonic`, `conservation_violations`, `conservation_start_total`/`conservation_end_total`, `brains_identical`, `channels_live`, `births`, `alive_count`, `civilization_emerged`); `--seasons` runs the seasonal-year-envelope benchmark (Stage 14, reports `seasonal_conservation_exact`, `seasonal_violations`, `seasonal_start_total`/`seasonal_end_total`, `seasonal_boundary_deposit`, `closed_monotonic`, `brains_identical`, `summer_noon`, `winter_noon`, `deposited`, `sunshine`, `seasons_emerged`; tune with `--seasonality`, `--seasons-per-year`, `--solar-deposit-rate`). CLI: `python3 -m domains.shell.world_driver --grid 64,32,64 --seed 42 --ticks 50 --every 5` (or `--evolution --generations N --population N --ticks-per-gen N`, or `--emergence`, or `--social`, or `--messages`, or `--structures`, or `--culture`, or `--memory`, or `--predation`, or `--territory`, or `--lifecycle`, or `--civilization`, or `--seasons`). Pure observability — adds no physics, mutates nothing beyond the live scene |
| Live Realm View | `realm_view.py`, `realm_live.py` | The observation surface — the benchmarks' counterpart. `render_frame` draws the day's skyline (sun position tracks `world.light`, `day D/N light X.XX noon N%`), every material cell with a per-cell grayscale energy heat, and ALL alive babies overlaid as `B` on the z-slice holding the most babies (glyphs ANSI-green when energized, yellow mid, red low). With the seasonal year envelope on it also names the current season and traces a year bar (Stage 14). `live_view` drives the REAL `Simulation.step` loop one tick at a time, clearing/redrawing per tick, and returns `{ticks, energy_total, solar_in, alive, population, births, deaths}`. `make_live_scene` generates the world, seeds food pools, and spawns `Genome.random` babies on the surface. CLI: `python3 -m domains.shell.realm_live --ticks 240 --day 24 --fps 8 --seed 7` (also `--grid X,Y,Z --population --pools --rate`, plus `--seasons [--seasons-per-year N] [--seasonality F]` for the Stage 14 year). Pure observability — adds no physics, mutates nothing beyond the live scene; stepping is re-seeded from `world_seed` so every run is reproducible |
| 23 Tests | `test_world_driver.py` | Material naming (derived from constants, never hardcoded), grid parsing, empty-vs-terrain populations, snapshot shape (incl. `nests` + `nest_energy` keys), per-tick cadence, energy-ledger consistency (incl. nest energy in the total), conservation monotonicity + violation detection, seed determinism, mean baby energy, evolution summary shape, emergence CLI path, seasons CLI path, CLI output paths — all pass |
...
8. **Life cycle** — populations self-sustain. Built (Stage 10, opt-in `--lifecycle`): a parent whose energy clears `reproduce_energy_threshold` and whose `perceptron_reproduce` gate clears `reproduce_gate_threshold` grants a birth inside the tick loop — the nearest nest of its tribe funds `birth_cost * birth_nest_fraction`, the parent pays the rest, and the child (a copy of the parent's brains and best memories, same tribe) spawns within `birth_range`. Birth is a transfer, never creation, so the world's energy still conserves; starvation deaths already swept the dead every tick, and births now replace them — a scene can grow and sustain its own population without the evolution engine re-seeding it. The gate starts at zero (exactly on the threshold) and selection decides whether opening it pays
